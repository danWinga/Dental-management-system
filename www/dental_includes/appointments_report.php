...
if( isset($_POST['token_apr1']) and isset($_SESSION['token_apr1']) and $_SESSION['token_apr1']==$_POST['token_apr1']){
	...
	$registered_priority=$unregistered_priority =$registered_doctor=$unregistered_doctor='';
	...
	if(!$exit_flag and isset($_POST['search_criteria_apr']) and $_POST['search_criteria_apr']=='date_range' and 
		...
		if($_POST['to_date']=='' or $_POST['from_date']==''){
			...
			//check if prority is set
			if($_POST['priority_criteria_apr'] != 'all'){
				$registered_priority = " and  registered_patient_appointments.priority =:priority";
				$placeholders[':priority']=$_POST['priority_criteria_apr'];
				
				$unregistered_priority = " and  unregistered_patient_appointments.priority =:priority";
				$placeholders[':priority']=$_POST['priority_criteria_apr'];
			}
			
			//check if doctor is set
			if($_POST['doc'] != 'all'){
				$registered_doctor = " and  registered_patient_appointments.doc_id =:doc_id";
				$placeholders[':doc_id']=$encrypt->decrypt($_POST['doc']);
				
				$unregistered_doctor = " and  unregistered_patient_appointments.doc_id =:doc_id";
				$placeholders[':doc_id']=$encrypt->decrypt($_POST['doc']);
			}
	...
	if(!$exit_flag and !$skip_registered ){
		...
		$sql="select registered_patient_appointments.appointment_date,  registered_patient_appointments.treatment, registered_patient_appointments.shour, 
			...
			registered_patient_appointments.priority
		...
		$date_criteria_registered $registered_priority	$registered_doctor ";
		...
		foreach($s as $row){
			...
			$appointment_array[]=array('date'=>"$date", 'doctor'=>"$doctor", 'patient'=>"$patient", 'phone'=>"$phone", 'treatment'=>"$treatment", 
				...
				'new_appointment_date'=>"$new_appointment_date",'smin'=>"$smin",'surgery_name'=>"$surgery_name", 'added_by'=>"$added_by", 'priority'=>$priority);
		}
	}
	...
	if(!$exit_flag){
		if(!$skip_unregistered){
			$sql="select unregistered_patient_appointments.appointment_date,  unregistered_patient_appointments.treatment, unregistered_patient_appointments.shour, 
				...
				unregistered_patient_appointments.priority
			...
			$date_criteria_unregistered $unregistered_priority $unregistered_doctor";
			...
			foreach($s as $row){
				...
				$appointment_array[]=array('date'=>"$date", 'doctor'=>"$doctor", 'patient'=>"$patient", 'phone'=>"$phone", 'treatment'=>"$treatment", 
					...
					'new_appointment_date'=>"$new_appointment_date",'surgery_name'=>"$surgery_name", 'added_by'=>"$added_by", 'priority'=>$priority);
			}
		}
	}
	...
	if(!$exit_flag ){
		if(count($appointment_array) > 0){
			...
			foreach($appointment_array as $row){
				...
				$bgcolor='';
				$task='';
				if($row['priority'] == 1){ $task = "<br>Priority: Low";}
				elseif($row['priority'] == 2){ $task = "<br>Priority: Medium";}
				if($row['priority'] == 3){ $task = "<br>Priority: High";}
				
				if($row['treatment']!=''){$task="$task <br>$row[treatment]";}
...
?>	
	<form class='' action="" method="POST" enctype="" name="" id="">
		...
		<div class='grid-100 apr_date_range no_padding'>
			<div class='grid-15'><label for="user" class="label">From </label></div>
			<div class='grid-10 '><input type=text name=from_date class=date_picker /></div>
			
			<div class='grid-5'><label for="user" class="label">To</label></div>
			<div class='grid-10 '><input type=text name=to_date class=date_picker /></div>
			
			<div class='grid-5'><label for="user" class="label">Priority</label></div>
			<div class='grid-10 '><select name=priority_criteria_apr class= >
			...
			<!--show doctor-->
			<div class='grid-5'><label for="" class="label">Doctor</label></div>
			<div class='grid-25'>
				<?php
				echo "<select name=doc ><option value='all'>ALL Doctors</option>";
					$sql=$error=$s='';$placeholders=array();

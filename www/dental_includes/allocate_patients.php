			...
			if($n > 0){
				foreach($current_allocations as $allocate){
					...
					if($registered_patient==1){
						...
						// check card current amount 
						$pt_carrent_card_amount = "N/A"; 
												
						
						//$_SESSION["pt_carrent_card_amount"]
						//if(isset($_SESSION[$appointment_pid])){unset($_SESSION["$appointment_pid"]);}
						//this is a molars registerd pt
						if(isset($_SESSION["$pid_bal"])){

							$pt_carrent_card_amount = "N/A1";
							$card_pid="$appointment_pid";
							$sql3cb=$error3cb=$s3cb='';$placeholders3cb=array();
							$sql3cb="select card_amount from patient_card_status where pid=:pid";
							$error3cb="Unable to select patient card status";
							$placeholders3cb[':pid']=$card_pid;
							$s3cb = select_sql($sql3cb,$placeholders3cb, $error3cb, $pdo);					
							if($s3cb->rowCount() > 0){
									foreach($s3cb as $row3ca ){
										$pt_carrent_card_amount=html($row3ca['card_amount']);
									}	

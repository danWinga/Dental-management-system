...
echo "<div class='grid_12 page_heading'>SURGERY NAMES</div>";
$user=$user_name=$var='';
...
if( isset($_POST['old_chair']) and $_POST['old_chair']!='' and $_SESSION['token_surgery2']==$_POST['token_surgery2']){
	...
	try{
		$pdo->beginTransaction();	
			...
			/*	//now delete entries  --nolonger applicable
			if(isset($_POST['del'])){
				$n=count($_POST['del']);
				$ins_id=$_POST['del'];
				$i=0;
...
?>
	...
	<input type=button value='Add New Surgery Unit' class='button_style pop_up_form'  />
	<div  id=patient_relationship_form_div title="New Dental Surgery Name">		
		<form action="" method="post" name="" id="">
			<div class='grid-25 alpha'><label for="user" class="label">Surgery Name </label></div><div class='grid-75 omega'><input type=text name=surgery_name /></div>
...
<?php
	...
	if($s->rowCount() > 0){
		$count=0;
		echo "<br><br><form action='' method='post' name='' id=''><table class='insurance_company'><caption>Surgery names</caption><thead>
		<tr><th class=rel_count></th><th class=rel_name>Surgery Name</th></tr></thead><tbody>";
		foreach($s as $row){
			$count++;
			...
			<input type=hidden name=ninye[] value='$val' /></td></tr>";
		}
		echo "</tbody></table>";
		echo "<br>";

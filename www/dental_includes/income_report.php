...
if(isset($_SESSION['token_ir1']) and isset($_POST['token_ir1']) and $_POST['token_ir1']==$_SESSION['token_ir1']){
	...
	if($_POST['branch'] != 'all'){
		$_POST['branch'] = $encrypt->decrypt($_POST['branch']);
		if($_POST['branch'] == 1){
			$branch_filter = " and branch = 1 " ;
			$branch_name="Lenana ";
		}
		elseif($_POST['branch'] == 0){
			$branch_filter = " and branch = 0 " ;
			$branch_name="Electicity House ";	
		}
	}
	...
	if(!$exit_flag and $income_report == 5){
		...
		if($pay_type!=10){
			$sql="select a.when_added, a.receipt_num, a.amount, a.tx_number , b.first_name, b.middle_name, b.last_name,
				...
				from payments a join patient_details_a b on a.pid=b.pid and a.pay_type=:pay_type and a.when_added >=:from_date	and a.when_added <=:to_date $branch_filter
			...
			if($pay_type==3 or $pay_type==4 or $pay_type==5 or $pay_type==6 or $pay_type==9 ){
				...
				foreach($s as $row){
					if($i==0){
						...
						echo "<table class='normal_table'><caption>$branch_name $caption</caption><thead>
	...
	elseif(!$exit_flag and $income_report==1 or $income_report==3 ){
		if($income_report==3 ){
			$from_date=html($_POST['from_date']);
			$to_date=html($_POST['to_date']);
			$caption="$branch_name NONE INSURANCE PAYMENTS BETWEEN $from_date and $to_date";
		}
		elseif($income_report==1){
			$from_date=html($_POST['from_date']);
			$to_date=html($_POST['from_date']);
			$caption="$branch_name NONE INSURANCE PAYMENTS ON $from_date";
		}		
		...
		$sql="select a.when_added, a.receipt_num, a.amount, a.tx_number , b.first_name, b.middle_name, b.last_name,
			...
			and a.invoice_id=0 and a.pay_type!=10 $branch_filter
			left join users c on a.created_by=c.id
			left join payment_types d on d.id=a.pay_type order by a.id";
	...
	//	exit;
		...
		$sql="select a.when_added, a.receipt_num, a.amount, a.tx_number , b.first_name, b.middle_name, b.last_name,
			...
			from payments a join patient_details_a b on a.pid=b.pid and a.pay_type=10 and a.when_added >=:from_date	and a.when_added <=:to_date
			and a.invoice_id=0 $branch_filter
			left join users c on a.created_by=c.id
			left join payment_types d on d.id=a.pay_type order by a.id";
		...
		if(count($credit_note_array) > 0){
			...
			foreach($credit_note_array as $row){
				if($i==0){
					$caption=strtoupper("$branch_name CREDIT NOTES");
				echo "<table class='normal_table'><caption>$caption</caption><thead>
		...
		//now get invoices raised on that day
			...
			$sql1="SELECT id,pid,invoice_number,when_raised,added_by FROM unique_invoice_number_generator WHERE 
				when_raised >=:from_date AND when_raised <=:to_date $branch_filter";
			...
			foreach($invoices_array as $row){
					...
					if($count==0){
						if($income_report==3){
							$caption=strtoupper("$branch_name TREATMENTS INVOICED BETWEEN $from_date and $to_date");
						}
						elseif($income_report==1){
							$caption="$branch_name TREATMENTS INVOICED ON $from_date";
						}	
		...
		//get invoices raised before  this period but paid in this period
			...
			$branch_filter2='';
			if($_POST['branch'] == 1){
				$branch_filter2 = " unique_invoice_number_generator.branch = 1 " ;
			}
			elseif($_POST['branch'] == 0){
				$branch_filter2 = " unique_invoice_number_generator.branch = 0 " ;
			}
			$sql="SELECT sum(amount) as amount_paid, concat(a.first_name,' ',a.middle_name,' ',a.last_name) as patient_name, invoice_id, 
					...
					left join unique_invoice_number_generator on unique_invoice_number_generator.id=b.invoice_id
					WHERE $branch_filter2 and b.when_added>=:from_date  and b.when_added<=:to_date and b.invoice_id > 0 group by invoice_id";	
			$placeholders[':from_date']="$from_date";
			$placeholders[':to_date']="$to_date";
	...
	elseif(!$exit_flag and $income_report==2 or $income_report==4 ){
		...
		//show total for patient category
		echo "<tr><td class=ic_1>TOTAL AMOUNT FOR MOLARS </td><td class=ic_2>".number_format($total_molars,2)."</td></tr>";
	...
	<form action="" method="POST" enctype="" name="" id="">
		<div class='grid-100 '>
			...
			<div class='grid-15 '><label for="" class="label">Search by</label>
				...
				//select branch
				$e_house = $encrypt->encrypt("0");
				$lenana = $encrypt->encrypt("1");
				echo "<div class='grid-5 label'>Branch</div>
					  <div class='grid-10'><select  name=branch><option value='all'>All</option>

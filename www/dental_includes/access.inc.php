		...
		//$pdo->beginTransaction();
			...
			foreach($s as $row){
				...
				foreach($s2 as $row2){
					...
					$sql3="insert into patient_details_b set id_number=:id_number, address=:address,
						city=:city, occupation=:occupation,weight=:weight, dob=:dob, referee=:referee,
							...
							when_added=:when_added,	gender=:gender,	photo_path=:photo_path, pid=:pid,
							 religion=:religion, patient_segment=:patient_segment , patient_source=:patient_source";
					...
					$placeholders3[':religion']=$row2['religion'];
					$placeholders3[':patient_segment']=$row2['patient_segment'];
					$placeholders3[':patient_source']=$row2['patient_source'];
				...
				//change pid in current allocations 
				change_pid_in_allocations($pdo, $row['pid'], $id);
			}
...
//this will get list of patient segment 
function get_segment($pdo) {// include 'db.inc.php';
	$sql=$error=$s='';$placeholders=array();		
	$sql="select id from patient_segment";
	$error="Error: Unable to get list of patient segment";
	$s = 	select_sql($sql, $placeholders, $error, $pdo);	
	foreach($s as $row){$_SESSION['patient_segment_array'][]=$row['id'];}
}

//this will get list of religion for patients
function get_religion($pdo) {// include 'db.inc.php';
	$sql=$error=$s='';$placeholders=array();		
	$sql="select id from patient_religion";
	$error="Error: Unable to get list of patient religion";
	$s = 	select_sql($sql, $placeholders, $error, $pdo);	
	foreach($s as $row){$_SESSION['religion_array'][]=$row['id'];}
}

//this will get list of patient source 
function get_patient_source($pdo) {// include 'db.inc.php';
	$sql=$error=$s='';$placeholders=array();		
	$sql="select id from patient_source";
	$error="Error: Unable to get list of patient source";
	$s = 	select_sql($sql, $placeholders, $error, $pdo);	
	foreach($s as $row){$_SESSION['patient_source_array'][]=$row['id'];}
}
...
function get_time_slots($pdo,$appointment_date, $hour, $min, $surgery){
	//echo " $appointment_date, $hour, $min, $surgery";
	//check when next appointment is for unregistered
	$next_hr=$next_min='';
	$sql=$error=$s='';$placeholders=array();
	//$sql="select rank, smin from unregistered_patient_appointments where appointment_date=:appointment_date and status='NOT SEEN' and rank >=:rank and smin >=:smin and surgical_unit=:surgical_unit order by rank,smin desc limit 1";
	$sql="select `rank`,smin from unregistered_patient_appointments where appointment_date=:appointment_date and status='NOT SEEN' and (`rank` * 60 + smin) >=:rank_smin and surgical_unit=:surgical_unit order by `rank`,smin desc limit 1";
	$placeholders[':appointment_date']="$appointment_date";
	$placeholders[':rank_smin']=($hour * 60) + $min;
	//$placeholders[':smin']="$min";
	$placeholders[':surgical_unit']="$surgery";
	...
	//echo "-un-$next_hr - $next_min";
	//check when next appointment is for registered
	$sql=$error=$s='';$placeholders=array();
	//$sql="select rank, smin from registered_patient_appointments where appointment_date=:appointment_date and status='NOT SEEN' and rank >=:rank and smin >=:smin and surgical_unit=:surgical_unit order by rank,smin desc limit 1";
	$sql="select `rank`, smin from registered_patient_appointments where appointment_date=:appointment_date and status='NOT SEEN' and (`rank` * 60 + smin) >=:rank_smin and surgical_unit=:surgical_unit order by `rank`,smin desc limit 1";
	$placeholders[':appointment_date']="$appointment_date";
	$placeholders[':rank_smin']=($hour * 60) + $min;
	//$placeholders[':rank']="$hour";
	//$placeholders[':smin']="$min";
	$placeholders[':surgical_unit']="$surgery";
	$error="Unable to get patients seen yesterday";
	$s = 	select_sql($sql, $placeholders, $error, $pdo);
	foreach($s as $row){
		//echo "xxxxx$row[rank]xxxx$row[smin]";
		if($next_hr == ''){
			$next_hr=$row['rank'];
			//$next_hr=$row['rank'];
			$next_min=$row['smin'];
		...
		if($next_hr > $row['rank']){
			$next_hr=$row['rank'];
			$next_min=$row['smin'];
			//echo "-reg1-$next_hr - $next_min";
		}
		elseif($next_hr == $row['rank'] and $next_min > $row['smin']){
			$next_hr=$row['rank'];
			$next_min=$row['smin'];
			//echo "-reg2-$next_hr - $next_min";
		}
	}
	//echo "-reg-$next_hr - $next_min";
	#number of 15min slots
	$slots=12;//default
	if($next_hr != ''){
		$t1 = ($next_hr * 60) + $next_min;
		$t2 = ($hour * 60) + $min;
		$slots = ($t1 - $t2)/15;
	}
...
function clear_patient() {
		...
		$_SESSION['em_phone']=$_SESSION['behalf_name']=$_SESSION['behalf_relationship']=$_SESSION['when_added']=$_SESSION['patient_segment']='';
		$_SESSION['gender']=$_SESSION['photo_path']=$_SESSION['email_address']=$_SESSION['email_address_2']=$_SESSION['religion']=$_SESSION['patient_source']='';
}
...
function get_patient($pdo,$criteria,$patient_number) {// include 'db.inc.php';
	...
	foreach($s as $row){
		...
		$_SESSION['religion']=html($row['religion']);
		$_SESSION['patient_segment']=html($row['patient_segment']);
		$_SESSION['patient_source']=html($row['patient_source']);
...
//change pid in current allocations after patient swap
function change_pid_in_allocations($pdo, $old_pid, $new_pid){
	//check if the old pid is in allocations and is not completed for today
	$sql=$error=$s='';$placeholders=array();
	$sql="select  id from patient_allocations where patient_type=1 and pid=:old_pid and discharge_time='0000-00-00 00:00:00' ";
	$placeholders[':old_pid']=$old_pid;
	$s = 	select_sql($sql, $placeholders, $error, $pdo);
	foreach($s as $row){
		//update pid
		$sql2=$error2=$s2='';$placeholders2=array();
		$sql2="update patient_allocations set pid=:new_pid where id=:id ";
		$placeholders2[':new_pid']=$new_pid;
		$placeholders2[':id']=$row['id'];
		insert_sql($sql2, $placeholders2, $error2, $pdo);
	}
}
...
function get_patient_basics($pdo,$pid,$encrypt){
			...
			if($s->rowCount()>0){
				foreach($s as $row){
					...
					// get patient current amount in insurance card
					$pt_carrent_card_amount = "N/A";
					$sql3=$error3=$s3='';$placeholders3=array();
					$sql3="select card_amount from patient_card_status where pid=:pid";
					$error3="Unable to select patient card status";
					$placeholders3[':pid']=$pid;
					$s3 = 	select_sql($sql3, $placeholders, $error3, $pdo);					
					if($s3->rowCount() > 0){
						foreach($s3 as $row3 ){$pt_carrent_card_amount=html($row3['card_amount']);}	
...
echo "<table>
	...
	<th>INSURANCE BALANCE</th><th>SELF BALANCE</th><th>POINTS BALANCE</th><th>cover limit</th><th>cover expiry</th><th>card Amount</th></tr></thead>
	<tbody><td class=spt>$patient_number</td><td>$first_name</td><td>$middle_name</td><td>$last_name</td>
	<td>$type_name</td><td>$company_covered_name</td><td>$data[0]</td><td>$data[1]</td><td>$data[2]</td><td>limit</td><td>expiry</td><td>$pt_carrent_card_amount</td></tbody></table>";
...
// check for patient religion and segment has been classified

function check_both_religion_and_segment($pdo,$pid,$encrypt){
	//select patient_details_b
	$result =array();
	$selected_id=$encrypt->decrypt($pid);
	$sql2=$error2=$s2='';$placeholders2=array();
	$sql2="select * from patient_details_b where pid=:pid";
	$error2="Unable to get patient details 2";
	...
	$sql3=$error3=$s3='';$placeholders3=array();
	$sql3="select * from patient_details_a where pid=:pid";
	$error3="Unable to get patient details 3";
	$placeholders3[':pid']=$selected_id;
	$s3 = 	select_sql($sql3, $placeholders3, $error3, $pdo);
	...
	foreach($s2 as $row2){
		...
		$sub_array['pid'] = $val2;
		$sub_array['segment'] = html($row2['patient_segment']);
		$sub_array['religion'] = html($row2['religion']);
		$result[] = $sub_array;	
...
// check if patient has segment classified


function check_selected_patient_segment($pdo,$pid){
	//select patient_details_b
	$sql2=$error2=$s2='';$placeholders2=array();
	$sql2="select patient_segment from patient_details_b where pid=:pid";
	$error2="Unable to get patient details 2";
	$placeholders2[':pid']=$pid;
	$s2 = 	select_sql($sql2, $placeholders2, $error2, $pdo);
	foreach($s2 as $row2){
		$patient_segment_in=html($row2['patient_segment']);

	}

	return $patient_segment_in;

}
// check for patient religion has been classified

function check_selected_patient_religion($pdo,$pid){
	//select patient_details_b
	$sql2=$error2=$s2='';$placeholders2=array();
	$sql2="select religion from patient_details_b where pid=:pid";
	$error2="Unable to get patient details 2";
	$placeholders2[':pid']=$pid;
	$s2 = 	select_sql($sql2, $placeholders2, $error2, $pdo);
	foreach($s2 as $row2){
		$patient_religion=html($row2['religion']);

	}

	return $patient_religion;
...
//check if this patient is already swapped
function check_if_swapped_patient_number($pdo,$criteria,$old_pid){
	//echo "$criteria,$old_pid";
	if($criteria=='pid' and $old_pid!=''){
			$sql=$error=$s='';$placeholders=array();
			$sql="select * from swapped_patients where old_pid=:old_pid";

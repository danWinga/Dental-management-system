...
if(isset($_POST['token_pl1']) and 	$_POST['token_pl1']!='' and $_POST['token_pl1']==$_SESSION['token_pl1']){
		$_SESSION['token_pl1']='';
		$exit_flag=false;
		$_POST['summary_detail']='detailed';//as select box with summary option is not used
		$sql=$error=$s='';$placeholders=array();
		$sql2=$error2=$s2='';$placeholders2=array();
		$insurer=$company = $email_filter=$gender_filter =$age_filter='';
		...
		$to_date=html($_POST['to_date']);
			...
			//gender criteria
			if(!$exit_flag and $_POST['gender_filter']!='all'){
				$gender_filter = " and c.gender=:gender ";
				$placeholders[':gender']=$_POST['gender_filter'];
			}
			
			//age criteria
			if(!$exit_flag and $_POST['age_filter']!='all'){
				if($_POST['age_filter'] == 'greater_than'){$age_filter = ' and ((year(now()) - year(c.dob)) >:age_input) ';}
				elseif($_POST['age_filter'] == 'less_than'){$age_filter = ' and ((year(now()) - year(c.dob)) <:age_input) ';}
				elseif($_POST['age_filter'] == 'equal_to'){$age_filter = ' and ((year(now()) - year(c.dob)) =:age_input) ';}
				$placeholders[':age_input']=$_POST['age_input'];
			}
		...
		if(!$exit_flag and isset($_POST['summary_detail'])){
			...
			if(!$exit_flag and $_POST['summary_detail']=='summary'   ){	
				$sql="select count(c.pid),b.name as name,b.id,a.pid from patient_details_b as c join patient_details_a as a  on c.pid=a.pid and 
					c.when_added >=:from_date and c.when_added <=:to_date $insurer $company $email_filter $gender_filter 
			...
			elseif(!$exit_flag and $_POST['summary_detail']=='detailed'   ){	
				$sql="select concat(a.first_name,' ',a.middle_name,' ',a.last_name) as patient_names, e.name as company_covered,b.name as insurer,
						a.biz_phone,a.mobile_phone,a.email_address,a.email_address_2,c.address,c.pid,c.when_added,a.patient_number,c.gender,c.dob,
						(year(now()) - year(c.dob)) as pt_age
					from patient_details_b as c join patient_details_a as a  on c.pid=a.pid and 
					c.when_added >=:from_date and c.when_added <=:to_date $insurer $company $email_filter $gender_filter $age_filter
				left join insurance_company as b on a.type=b.id
				...
				if($s->rowCount() > 0){
					...
					foreach($s as $row){
						//location filter
						if($_POST['location_filter']!='all'){
							//check if the pt has any receipt from lenana
							$sql2=$error2=$s2='';$placeholders2=array();
							$sql2="SELECT id from payments where pid=:pid and receipt_num like 'L%'";
							$error2="Unable to get covered companies under summary patient registration report";
							$placeholders2[':pid']=$row['pid'];
							$s2 = 	select_sql($sql2, $placeholders2, $error2, $pdo);
							$lenana_receipt_count = $s2->rowCount() + 0;
							
							//check if the pt has any invoice from lenana
							$sql2=$error2=$s2='';$placeholders2=array();
							$sql2="SELECT tplan_id from tplan_procedure where pid=:pid and invoice_number like 'L%'";
							$error2="Unable to get covered companies under summary patient registration report";
							$placeholders2[':pid']=$row['pid'];
							$s2 = 	select_sql($sql2, $placeholders2, $error2, $pdo);
							$lenana_invoice_count = $s2->rowCount() + 0;
							
							//echo "<br>$_POST[location_filter]-$lenana_receipt_count-$lenana_invoice_count<br>";
							if($_POST['location_filter'] == 'lenana' and $lenana_receipt_count == 0 and $lenana_invoice_count == 0){continue;}
							if($_POST['location_filter'] == 'e_house' and ($lenana_receipt_count > 0 or $lenana_invoice_count > 0)){continue;}
						...
						if($count==0){
							...
							<th class=srpd_pname>GENDER</th>
							<th class=srpd_pname>DOB</th>
						...
						$dob=html($row['dob']);
						$pt_age=html($row['pt_age']);
						
						echo "<tr ><td>$count</td><td>$registered</td><td>$patient_name</td><td>$gender</td><td>$dob<br>$pt_age</td>
						<td>$patient_number</td><td>$patient_type</td>
	...
	?>
		...
		<?php $token = form_token(); $_SESSION['token_pl2'] = "$token";  ?>			
				...
				<div class='no_padding registration_div'>
					...
					<div class='grid-20'><label for="" class="label">Select gender</label>
					</div>
					<div class='grid-10'><select name=gender_filter>
					...
					<div class='prefix-15 grid-15'><label for="" class="label">Select location</label>
					</div>
					<div class='grid-10'><select name=location_filter>
						<option value='all'>Any</option>
						<option value='lenana'>Lenana</option>
						<option value='e_house'>E. House</option>
						</select>
					</div>
					<div class=clear></div><br>
					
					<div class='grid-20'><label for="" class="label">Select Age</label>
					</div>
					<div class='grid-10'><select name=age_filter>
						<option value='all'>Any</option>
					...
					<div class='grid-5'><input type=text name='age_input' /></div>
					
					<!--<div class='prefix-10 grid-15'><label for="" class="label">Select report type</label>
					</div>

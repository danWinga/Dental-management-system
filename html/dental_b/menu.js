...
$('.procedure_container').on('click', 'input[name^=codex]', function(){
	//$('input[name^=codex]').click(function(event) {
		event.preventDefault();
		//this.blur(); // Manually remove focus from clicked link.
		var dat = 'testing';
		window.codex_id= $(this).attr('id');
	
		$('input[name^=procedure]').removeAttr('selected').find('option:first').attr('selected','selected');
		...
		var myoutput = 'myOutPut';
				...
				$.ajax({
					url: 'treatment-plan/encrypt_id.php',
					type:"POST",
					data:{action:action, myOutPut: myoutput },								
					...
					sessionedProcedureId(outputfromserver);
								...
								$('#tst').dialog({
									title: 'SELECT TREATMENT PROCEDURES',
									height: height_y,
									width: width_x,
									show:true,
									modal: true,
	...
	{
		$.ajax({
		url:"dental_procedure_upgrades/dist/fill_treeview.php",
		method:"POST",
		data:{query:query},
		success:function(data)
		{
	...
	function sessionedProcedureId(outputfromserver){
		var data = JSON.parse(outputfromserver);
		//alert("all procedure ids1 :"+ outputfromserver);
		//alert("all procedure idsobj :"+ dataProcObj);
		return data;
...
function fill_treeview2(query)
   {
	   $.ajax({
		   ...
		   success: function(data){
			...
			var treeView= $('#treeview2').treeview({
				   data:data,
				   multiSelect: false,
				   ...
				   onNodeSelected: function(event, node) {
					   ...
					   collapseNode(node.nodeId);
						  ...
						  // alert("node selected"+ node.nodeId +": \n Node text:"+ node.text +": \n Node nodes:"+ node.id+": \n Node code:"+ node.code);
						   ...
						   //alert("selected item value"+ node.id +": \n result_text:"+ window.result_text );
						   ...
						   //all_procedure_id(window.result_value ,window.result_text);
			   ...
			   $('#treeview2').treeview('collapseAll', { silent: true });
   ...
   function collapseNode(nodeId) {
	   $('#treeview2').treeview('collapseNode', [nodeId]);
	   //alert("collapse " + nodeId);
   }

   function expandNode(nodeId) {
	   $('#treeview2').treeview('expandNode', [nodeId]);
  ...
  function fill_treeview3(treeData)
   {
	//var treeData = trview();	
			var tree = $('#treeview2').treeview({
			   ...
			   tree.treeview('clearSearch');
				...
				// treeview seach functions --- start

				var selectors = {
					'tree': '#treeview2',
					'input': '#pro_search_text',
					'reset': '#pro_search_btn'
				};
				...
				var search = function (e) {
					...
					if (pattern.length < 3) { // avoid heavy operation
						...
						tree.disableNode(unrelated, {silent: true}).hide();
						//tree.hideNode(unrelated, {silent: true});
					}
				};
	...
	{	
		//alert ("my ivalue "+result_value + "text: "+result_text);
		var data = dataProcObj;
		//alert("response  id data :"+ data[0].id +" ==Iden:"+ data[0].iden );
		var currentId = "";
		var currentEncrypt ="";
		var parent = 0;
		var foundId = 0;
		for (var i = 0; i < data.length; i++) {
			...
			currentId = parseInt((data[i].id).trim());
			if (currentId === parseInt(result_value)){
				currentEncrypt = data[i].iden;
				parent = parseInt((data[i].parent).trim());
				foundId = currentId;
				break;							
		...
		//alert("response  id data  :"+ currentId +" ==Parent:"+ parent + "result text =:"+ result_text );
		if (parent > 0){
			//$('#procedurex').val(result_text);
			$('#'+ codex_id).val(result_text);					
			$('#procedure'+ codex_id).val(currentEncrypt);
			$('#codex2'+ codex_id).val(currentId);
			getTeeths(currentEncrypt);
			$('#tst').dialog('close');
	...
	var currentEncrypt = 0;
	function getTeeths(currentEncrypt){
		//alert ("add_procedure ---: currentEncrypt "+ currentEncrypt);
		
		//alert('998');
		$('#pay_method'+ codex_id).prop('disabled', false);	
		$('#tplan_alias'+ codex_id).prop('disabled', false);
		
		var add_procedure = currentEncrypt;
			//alert('996');
		//empty other fields when procedure chnges
		$('#details'+ codex_id).val('');
		$('#pay_method'+ codex_id).val('');
		$('#cost'+ codex_id).val('');
		$('#tplan_alias').attr('checked', false);
		get_treatment_plan_total_cost();
	//this will ensure that in tplan only aviallable fields are not disabled
		//alert('value ismmmmmmmmmmmmmm ' + currentEncrypt);
		if( currentEncrypt !='' ){
			//details
			$('#details'+ codex_id).prop('disabled', false);
			//payment method
			$('#pay_method'+ codex_id).prop('disabled', false);		
			$('.tplan_alias').prop('disabled', false);	 
			$('#cost'+ codex_id).prop('disabled', true);
						
		}
		else{
			$('#details'+ codex_id).prop('disabled', true);
			$('#pay_method'+ codex_id).prop('disabled', true);
			$('#cost'+ codex_id).prop('disabled', true);
			$('.tplan_alias').prop('disabled', true);			

		}

element = $(this);
	//alert('vals isadddddddd ' + add_procedure);
	...
	//check if the procedure is in loyalty program
		...
		pay_method_select = $('#pay_method'+ codex_id);
	//	alert('vals isadddddddd ' + pay_method_select);
		$.ajax({
		type: "POST",
		url: "dental_b/",
		data: form_data,
	...
	//check if the procedure is covered by insurance
		...
		success: function(data) {
		//	alert("check if the procedure is covered by insurance: "+data);
		  var x2 = data.split('#');	
		  if (x2[0] == 'not_covered') {
				$('option ' ,  pay_method_select).each(function(){
					//alert($(this).text())
		...
		success: function(data) {
		//alert('show teeth data is  ' + data);
		//alert('show teeth div id:  ' +$('#teeth_div'+codex_id).attr('id'));
		//var teeth_div = $('#teeth_div'+codex_id).attr('class');
		var teeth_div_id = $('#teeth_div'+codex_id).attr('id');
		//alert('show teeth div id:  ' + $('.teeth_div'+codex_id));
			if (data == 'show_teeth') {$('.teeth_div'+codex_id).addClass('teeth_body').slideDown("slow");}
			else if (data == 'do_not_show_teeth') {$('.teeth_div'+codex_id).
													slideUp("slow").
...
return false;
	...
	function load_valuesxxx(result_value ,result_text)
	{
	...
	function all_procedure_id(result_value ,result_text)
	{
			//alert("div id:"+ result_text + "oyyy id:"+ result_value);
			...
			$.ajax({
				...
				success: function(outputfromserver){
					//console.log("Index value"+ outputfromserver);
					
					//alert("response  id procedure :"+ outputfromserver);
					//alert("Tree veiw Id  :"+ result_value);
					var data = JSON.parse(outputfromserver);
					//alert("response  id data :"+ data[0].id +" ==Iden:"+ data[0].iden );
					var currentId = "";
					var currentEncrypt ="";
					...
					alert ("my id is: "+foundId +" ==my encryption is"+currentEncrypt);
...
$('#addTreexx').click(function(e){
	e.preventDefault();
		var addTree = $(this).text();
...
//if($(this).hasClass('token_g2_patient')){alert ('yyyyyyyyyyyyyyyyyyyyyyyyyy');}
	...
	$.post("dental_b/", $(this).serialize())
	 .done(function(data) {
	//alert('data isvwhat winga ' + data);
	//alert('data isvwhat winga3 ' + action);
	if(data =='good#on-examination#Patient details saved.'){//good#on-examination#Patient details saved.
		//alert(' Winga action is what action: ' ); //on-examination
		$('#ui-tabs-7').load("treatment-plan/index.php");
				//$('#ui-tabs-7').load("examination/index.php");
				$('html, body').animate({scrollTop: '0px'}, 500);
				//$('#ui-tabs-7').show();
				//$('#ui-tabs-7.active').tab('show');
	}
	//alert('action is what action: ' + action);
	if(data =='good#patient_allocation#Patient discharged'){
		//alert('we are discharging: ' + action);
		window.location = "?id=allocate-patients";// id=allocate-patients	
	}
	if(data =='good#patient_allocation#Waiting list updated'){
		//alert('we are Waiting list updated: ' + action); 
		window.location = "?id=allocate-patients";// id=allocate-patients	
	}
	if(data =='good#authorise_invoice#Changes saved'){
		//alert('good#authorise_invoice#Changes savedffffffffffff: ' + action); 
		window.location = "?id=invoice-authorisation";// id=allocate-patients	
	}
	if(data =='good#invoice-payments#Payments saved'){
		//alert('good#authorise_invoice#Changes savedffffffffffff: ' + action); 
		window.location = "?id=insurance-payments";// id=allocate-patients	
	}
	
	if(data =='good#go_to_examination'){
		//alert('we are examination: ' + action );
		window.location = "?id=patient#examination";// id=allocate-patients	
		//$('#'+ tab_id).load("treatment-plan/index.php");
		 // $('#ui-tabs-7').load("examination/index.php");
		  //$('html, body').animate({scrollTop: '0px'}, 250);
		 
	}
	if(data =='good#return_to_allocation'){
		//alert('we are return_to_allocation: ' + action);
		window.location = "?id=allocate-patients";// id=allocate-patients	
	}

	if(data =='good#self_payment#Payment saved'){
		//alert('we are return_to_allocation: ' + action);
		window.location = "?id=self-payments";// id=allocate-patients	
	}
	// added new
	if(data =='good#Patient disease details saved'){
		//alert('we are return_to_allocation: ' + action);
		window.location = "?id=patient";// id=allocate-patients	
	}
	if(data =='good#patient_allocation#Patient allocated'){
		//alert('we are return_to_allocation: ' + action);
		window.location = "?id=patient";// id=allocate-patients	
	}
	if(data =='good#on-examination#Patient details saved.'){
		//alert('we are return_to_allocationxxxxxxxx: ' );
		window.location = "?id=patient#treatment-plan";
		//window.location = "?id=patient";// id=allocate-patients
		//c$('#'+ tab_id).load("treatment-plan/index.php");
				//$('#ui-tabs-7').load("examination/index.php");
			//c	$('html, body').animate({scrollTop: '0px'}, 250);
				//$('html, body').animate({scrollTop: '0px'}, 500);	//treatment-plan examination
	}

	
	
	
	if(data =='bad#No such patientbad#No such patient'){
		//window.location = "?id=allocate-patients";// id=allocate-patients
		//alert('bad#No such patientbad#No such patient: ' + action);
		var error ="No such patient"
		$('.feedback').empty()
	...
	if(data =='bad#No such patient'){
		//window.location = "?id=allocate-patients";// id=allocate-patients
		//alert('bad#No such patient: ' + action);
		var error ="No such patient"
		$('.feedback').empty()
			.removeClass('success_response hide_element ')
			.addClass('error_response')
	...
	//alert('data is lengthxxx ' + data.length + 'action :'+ action) ;
	/** 
	if(data.length = 43){
	...
	*/
	...
	//////////////////////////////////////////////////////////////////////////
		// start updating segment and religion  using patient_Number

	if (data.length > 2 && data.length < 1000  && action =='' ){
			//alert('data is length ' + data.length);
		var datax ='';
		...
		for (var i = 0; i < datax.length; i++) {
			...
			pid_pt  = datax[i].pid;
			segment = datax[i].segment;
			religion = datax[i].religion;
			type = datax[i].type;		
												
		}
		pbUdpID =  pid_pt;
		//alert('data is  infor: pid:'+ pid_pt + 'religion:'+ religion + ': segment :'+ segment + 'Patient_Type:'+ type  );
		if(pid_pt !='' && (religion !='' || segment !='') ){
			//alert('data is  infor: pid:'+ pid_pt + 'religion:'+ religion + ': segment :'+ segment  );
			
			//alert('patient is ready for allocation------vvvvvvvvvvvvvvvvvv-' );
			var udpId = pid_pt; 
			//pbUdpID =  pid_pt;
			//allocate_patient_to_waiting_list(udpId);
			if(type >3){
				//alert('patient is insurance type-ccccc-----:'+ type );
				fill_missing_pt_contact(pid_pt);
				fill_religion_segment(pid_pt);	

			}else{
				//alert('patient is cash typexxxxxxx------:' +type );
				var udpId = pid_pt;									
				allocate_patient_to_waiting_list(udpId);
		...
		if( pid_pt !='' && (segment =='' || religion =='')){
			
			// alert('patient is segment and religion is empty----CCCCCCCCCC' );
			fill_missing_pt_contact(pid_pt);
			fill_religion_segment(pid_pt);
			//update_patient_changes(udpId );
		...
		// end  updating segment and religion
	/////////////////////////////////////////////////////////////
	...
	//	$('.show_loader').dialog('close');
		...
		if(x[0] == "good"){
			...
			else if(action == "#patient_allocation#Waiting list updated"){
				//var undispatched_finished_labs = 'yes';
				//$('#undispatched_labs').load('dental_b/', {'undispatched_finished_labs': undispatched_finished_labs });
				//alert('action is vvvvvvvvvvv' + action);
				
			}
			else if(action == "#patient_allocation#Patient discharged"){
				//var undispatched_finished_labs = 'yes';
				//$('#undispatched_labs').load('dental_b/', {'undispatched_finished_labs': undispatched_finished_labs });
				//alert('action is vvvvvvvvvvvsssdsdsds' + action);
				
			}

		//alert('action isbbb ' + action);
		...
		//	else{
			if(typeof x2[1] !== 'undefined'){
				...
				$('.selected_pt').click(function() {
					...
					if(selected_pt !==''){
						var udpId = selected_pt;
						var v12345;
						var pid_pt = selected_pt;
						

						var action = "check_upd_action";
						$.ajax({
							method: "GET", 
							url: "dental_b/", 			   
							dataType: "json",
							data:{ selected_upd:  udpId , action: action},
							success: function(response){
								
							var recordFound = response;
							
							//var data = JSON.parse( recordFound);
							
							//alert('found patient data is  ' + recordFound);
							var datax ='';
							var datax = recordFound;
							
							//var datax1 = JSON.parse( recordFound);
							//alert('found  response  ' + recordFound);
							var pid_pt  ='';
							var segment = '';
							var religion = '';	
							var type = '';								
							...
							pbUdpID =  pid_pt;
								if(pid_pt !='' && (religion !='' || segment !='') ){
									...
									var udpId = pid_pt; 
									if(type !=3){
										//alert('patient is insurance type------:'+ type );
										$('.div_shower').dialog("close");
										fill_missing_pt_contact(pid_pt);
										fill_religion_segment(pid_pt);	
						
									}else{
										//alert('patient is cash type------:' +type );
										//var udpId = pid_pt;		
										$('.div_shower').dialog("close");							
										allocate_patient_to_waiting_list(udpId);
...
function fill_religion_segment(pid_pt){
	//alert('show dialog  box' );
	//clear_patient_dg();
	//fill_missing_pt_contact(pid_pt);
	var width_x = $('.get_width').show().width();
	$('#patient_other_details_update').dialog({
...
function fill_patient_dg(fname,mdname,lname,gender, religion,segment){
	//alert('Function call found patient record : firstnameSEGMENT : '+ segment + " midnameReligion: "+ religion + " lastname :"+ lname );
								
	$("#first_name2").val(fname);
	$("#middle_name2").val(mdname);
	$("#last_name2").val(lname);
	$("#gender2").val(gender);
	$("#patient_religion2").val(religion).attr('selected','selected');
	$("#patient_segment2").val(segment).attr('selected','selected');
...
function fill_pt_bal_status(udpId){

	//alert("please wait for response >>> checking for ---------- patient balance: ----------------------------------------------- "+udpId );
	var udpIdx = udpId;
	var action  = 'patient_status';
	$.ajax({
		method: "GET",
		dataType : "html", 
		url: "patient-contacts/patient_other_contact.php", 			   
		
		data:{ selected_upd: udpIdx , action: action},
		success: function(data){
			//alert("response testing&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&" + data);
			//alert(response);
			
			$("#patient_card_balx").html(data);
			//var result = $('<div />').append(data).find('#patient_card_bal').html();
			//$('#patient_card_balx').html(result);
			//$('#patient_card_balx').html($('#patient_card_balx',data).html());
		},
...
function clear_patient_dg(){
	$("#first_name2").val('');
	$("#middle_name2").val('');
	$("#last_name2").val('');
	$("#gender2").val('');
	$("#patient_religion2").val();
	$("#patient_segment2").val();
...
function allocate_patient_to_waiting_list(udpId){
	var action = "allocate_pt_record";
	var selected_upd = udpId;
	//alert('Function call for allocating patient to waiting list'+ udpId );
	...
	$.ajax({
		method: "GET",  
		 url: "dental_b/", 			   
		 dataType: "json",
		 data: {action:action, selected_patient:selected_upd},
		 success: function( response){
			setTimeout(function(){
				//alert('patient allocated correctly' + response);
				$('.div_shower').dialog("close");
				window.location = "?id=allocate-patients";
			}, 100);
		},
		...
		error: function(){
			setTimeout(function(){
				//alert('error in patient allocated ');
				window.location = "?id=allocate-patients";
			}, 100);
		}
...
function update_insurance_card_bal(udpId,new_bal){
	...
	if(intCard_bl){
		...
		$.ajax({
			...
			error: function(){
				setTimeout(function(){
					alert('error in updating patient insurance balance ');
...
function reload_page_on_submit(){
	$('.div_shower').dialog("close");
...
function ok_add_pt_to_waiting_list(pid_pt){
	var selected_patient = pid_pt;
	var action = 'allocate_pt_record';
	...
	$.ajax({
		...
		data:{ selected_patient: selected_patient , action: action},
		success: function(response){
			alert('patient allocated correctly: '+ response);
...
var pbUdpID = '';
//alert('found pdUdpID : '+ pbUdpID );
var pbSegment ='';
var pbReligion = '';
$('#pt_cancel_changes').click(function() {
	$('#patient_other_details_update').dialog("close");
 ...
 $('#patient_religion2').click(function(){ 
	
	 var val = $(this).val();
	 //pbSegment = vx;
	 pbReligion = val;
    //alert('text val :'+ vx + 'val:' + val);
  });

  $('#patient_segment2').click(function(){ 
	
	var val = $(this).val();
	pbSegment = val;
	//pbReligion = bc;
   //alert('text valxx :'+ vx + 'val:' + val);
 });

 //update patient  insurance card bal
 ...
 $('#pt_update_pt_bal').click(function() {
...
$('#pt_update_changes').click(function() {
	//var pbUdpID = '';
	//var selected_pt = udpId
	var element = $(this);
	
	var selected_pt = pbUdpID;
	//alert('found pdUdpIDxxx : '+ pbUdpID );
	var actionx ="update_pt_record";

	//alert($('#patient_segment2').find('option:selected').attr('id'));
	//var segmentx =$('#patient_segment2').val();
	//var religionx = $('#patient_religion2').val();
	//var segmentx = $('#patient_segment2').val();
	//var religionx = $('#patient_religion2').val();
	//alert(' segment val ::'+ segmentx + 'religionx::' + religionx);

	var segmentx  = pbSegment ;
    var religionx = pbReligion;

	//alert('found patient rec are: segment : '+ segmentx + " religion: "+ religionx + "selected_id: "+selected_pt );

	if(segmentx !='' && religionx !=''){
		var udpid =selected_pt;
		var  udpId = selected_pt;
		var resp = false;
		$.ajax({
			...
			data:{ selected_upd: selected_pt , religion_upd : religionx , segment_upd: segmentx,action: actionx},
			}).done(function(response){ //
				//$("#server-results").html(response);
				//alert('record response: '+ response);
				//alert('allocate Pt is : '+ udpId);
		...
		//alert('record ikosawa ama: '+ response);
		if(segmentx =='' && religionx ==''){
			alert('Both Religion and Segment not selected');

		}
		else if(religionx ==''){
			alert('Religion not selected');
		}
		else if(segmentx ==''){
			alert('Segment  not selected');
		}
	}
...
function update_patient_changes(udpId ){
...
function fill_missing_pt_contact(pid_pt){	
	var udpId = pid_pt;	
	//alert('udpId  ' + udpId);								
	var action = "fill_pt_record";
	//var selected_pt = selected_pt; //selected_pt selected_upd
	// url: "treatment-plan/encrypt_id.php",
	$.ajax({
		method: "POST",
		dataType: "json", 
		...
		success: function(response){
			...
			//alert('pole record empty  ' );
			var record = response;
			//alert('found  responseccc  ' + record);
			//alert('found  firstname  ' + record.firstName)
			var fname  ='';
			var mdname = '';
			var lname = '';
			var gender = '';
			...
			for (var i = 0; i < record.length; i++) {		
				fname  = record[i].firstName;
				mdname = record[i].middleName;
				lname  = record[i].lastName;
				gender  = record[i].gender;	
				religion  = record[i].religion;
				segment = record[i].segment;				
			}
			clear_patient_dg();
			 pbUdpID = udpId;
			//alert('found patient record are: firstname : '+ fname + " midname: "+ mdname + " lastname :"+ lname + " religion :"+ religion + " :segment :"+ segment   );
			fill_patient_dg(fname,mdname,lname,gender,religion ,segment);// fname,mdname,lname,gender, religion,segment
			//check_seg_and_relig(segment,religion);
						
			//alert('fin---------------------------fill_pt_bal_status: ' + udpId  );
			fill_pt_bal_status(udpId);
	...
	//alert('found patient record are: firstname : '+ fname + " midname: "+ mdname + " lastname :"+ lname );
			
	//xxpt_update_changes
	
	
			
			//alert('record ikosawa res Status44: '+ resStatus);
		...
		//	else{
			if(typeof x2[1] !== 'undefined'){
				...
				$('.selected_pt').click(function() {
					...
					if(selected_pt !=''){
						//alert('selected isxxxx  ' + selected_pt);
						element.append('<input type=hidden name=selected_patient class=selected_patient_input value=' + selected_pt + ' />');
						element.submit();
						$('.div_shower').dialog("close");
					}
...
$("a.link_clor" ).on( "mouseenter", function() {
 ...
 $('.div_shower44').on('change','.appointment_type',function(){
	var val= $(this).val();
	if(val == 'school_holiday'){
		$('.select_date, .show_current_appointments, #tdone_appointment_times').slideUp('fast');
		$('.holiday_class').slideDown('fast');
	}
	else if(val == 'normal'){
		$('.holiday_class').slideUp('fast');
		$('.select_date, .show_current_appointments, .tdone_appointment_times').slideDown('fast');
...
$('.tdone_appointment').click(function(e){
	...
	$(".div_shower44").empty().css('backgroundColor',' #15212F').dialog({
		...
		width: width_x,
		close: function( event, ui ) {
			feedback_div.empty().removeClass('error_response').removeClass('success_response');
...
$('.div_shower44').on('change','.tdone_appointment_date3',function(){
	...
	if(tdone_appointment_date5 == ''){alert('Please specify the appointment date');}
		...
		//$('#tdone_appointment_times').show().load('dental_b/', {'tdone_appointment_date': tdone_appointment_date5 });
		//get appointments for that doctor on that day
...
//$('.procedure_container2').on('click', 'input[name^=codex11]', function(){

	//alert("testing data codex11xxxxxxccccc");
//});
...
$('.procedure_container2').on('click', 'input[name^=codex]', function(){
	//$('input[name^=codex]').click(function(event) {
		event.preventDefault();
		//this.blur(); // Manually remove focus from clicked link.
		var dat = 'testing';
...
$('.procedure_container2xxxxx').on('click', 'input[name^=ccxxxxcodex]', function(){
	event.preventDefault();
		//this.blur(); // Manually remove focus from clicked link.
		var dat = 'testing';
		window.codex_id= $(this).attr('id');
		...
		var width_x = $('input[name^=codex]').parent().width();
		var height_y = 580;//$("#employer_form_div").height();
		alert("testing data codex11xxxxxxccccc:" +codex_id +" :treeData:"+ treeData );
		var loaderx1 = fill_treeview3(treeData);
		//var loaderx2 = fill_treeview2();
		window.loadTree =loaderx1
			...
			*/
			...
			$('#tst').dialog({
				...
				body: function(loaderx1){ // loaderx1
						window.setTimeout(function(){
							//loaderx1 = fill_treeview2();
							alert("testing dialog inside:" +codex_id);
...
$('.procedure_container2yess').on('click', 'input[name^=yescodex]', function(){
	//$('input[name^=codex11xxxxxxxxx]').click(function() {
		event.preventDefault();
		//this.blur(); // Manually remove focus from clicked link.
		var dat = 'testing';
		window.codex_id= $(this).attr('id');
...
$(this).parent().next().next().next().children('.tplan_alias').prop('disabled', false);	 	
		...
		get_treatment_plan_total_cost();
	//this will ensure that in tplan only aviallable fields are not disabled
		//alert('value ismmmmmmmmmmmmmm ' + $(this).val());
		if( $(this).val() !='' ){
			//details
			$(this).parent().next().children('.tplan_details').prop('disabled', false);
			//payment method

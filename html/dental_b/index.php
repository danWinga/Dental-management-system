...
$time_slot_array[1]= '15 min';
$time_slot_array[2]= '30 min';
$time_slot_array[3]= '45 min';
$time_slot_array[4]= '1 hour';
$time_slot_array[5]= '1 hr 15 min';
$time_slot_array[6]= '1 hr 30 min';
$time_slot_array[7]= '1 hr 45 min';
$time_slot_array[8]= '2 hours';
$time_slot_array[9]= '2 hrs 15 min';
$time_slot_array[10]= '2 hrs 30 min';
$time_slot_array[11]= '2 hrs 45 min';
$time_slot_array[12]= '3 hours';
...
elseif(isset($_POST['check_procedure_in_points']) and $_POST['check_procedure_in_points']!=''){
	...
	//$procedure_id=$_POST['check_procedure_in_points'];
...
$_POST['token_set_re_app1']==$_SESSION['token_set_re_app1'] and userHasRole($pdo,80)){
	...
	if($_POST['doctor']==''){
		$exit_flag=true;
		$message="bad#Please select the doctor for the re-appointment";
		#$message="bad#Please select the doctor for the re-appointment";
	}
	if($_POST['appointment_duration']==''){
		$exit_flag=true;
		$message="bad#Please select the duration for the re-appointment";
	}
	if(!$exit_flag){
		try{
			$pdo->beginTransaction();
				...
				#get prioroity from initial appointment
				$sql=$error=$s='';$placeholders=array();
				$sql="select  priority , treatment from $registered where id=:appointment_id";
				$placeholders[':appointment_id']=$old_appointment_id;
			...
			//	echo "$_SESSION[new_appointment2] -- 1437";
				...
				$placeholders[':duration']=$_POST['appointment_duration'];
				$placeholders[':priority']=$old_priority;
				$placeholders[':treatment']="$treatment";
...
//echo "$_SESSION[new_appointment2] -- $_SESSION[id] -- $_SESSION[logged_in_user_names] -- 1497";
	...
	<div id=registered_appointment>
		<?php
			...
			elseif($data[2]=='NO'){$registered=" unregistered_patient_appointments ";}
			$pid=$data[3];
			#get time slots from initial appointment
			$old_duration=1;
			$sql=$error=$s='';$placeholders=array();
			$sql="select duration , priority, treatment from $registered where id=:appointment_id";
			$placeholders[':appointment_id']=$old_appointment_id;
			$error="Unable to get patients seen yesterday";
			$s = 	select_sql($sql, $placeholders, $error, $pdo);
			foreach($s as $row){
			...
			$start_min = html("$data[1]");
			$slots = get_time_slots($pdo,$_SESSION['appointment_date'], "$data[0]", "$data[1]", "$data[2]");
			$initial_duration = $old_duration * 15;
			$new_duration = $slots * 15;
			if($slots < $old_duration){
				echo "<div class='grid-100   error_response'>Initial appointment had a duration of $initial_duration minutes, the new appointment time cannot exceed $new_duration minutes</div>";
			}
		...
		?>
			...
			<form class='patient_form check_selected_patient' action="" method="post" name="" id="">
				...
				echo "</select></div>";
				echo "<div class='grid-15 label'>Start Time:</div><div class='grid-10 label'>$start_hr:$start_min $start_am_pm</div>";
				
				//echo "<br>$slots -- $data[0]:$data[1] -- $next_hr:$next_min";
				echo "<div class='prefix-5 grid-10 label'>Duration</div><div class='grid-15 label'>";
					echo "<select name=appointment_duration><option></option>";
						$i3=1;
						while ($i3 <= $slots){
							$selected ='';
							if($old_duration == $i3){$selected = " selected ";}
							echo "<option $selected  value=$i3>$time_slot_array[$i3]</option>";
							$i3++;
						}
					echo "	</select>
...
elseif(isset($_POST['extra_procedure']) and $_POST['extra_procedure']!='' ){
				...
				echo "<div class='grid-100 tplan_procedures hover '>";
					...
					echo "<div class='grid-45 grid-parent'>";
						$sql2=$error2=$s2='';$placeholders2=array();
						$sql2="select name,id,parent_category_id, all_teeth from procedures where listed=0 order by name";
					...
					echo "</div>";
					echo "<div class='grid-25'><textarea   rows='' id=details$i name=details$i ></textarea></div>";
					echo "<div class='grid-15'>";
						...
						echo "</select>";
					echo "</div>";
					echo "<div class='grid-10'><input type=text class=tplan_cost id=cost$i  name=cost$i />";
...
//sleep(5);
	...
	if(!$exit_flag){
		try{
			...
			$tx_result = $pdo->commit();
			if($tx_result){
				$record_found = "on-examination";
				$message="good#on-examination#Patient details saved.";
				...
				if($_POST['hold_finish']=='transfer' ){
					if($_POST['transfer_list']==''){$message='bad#Please select the surgery to transfer the patient to.';}
						...
						try{
							...
							if($s){
								$tx_result = $pdo->commit();
								if( !$exit_flag and isset($tx_result) and $result ==1){
							...
							elseif(!$s){$pdo->rollBack();
								$tx_result=false;
								$message="bad#Unable to save patient disease details  ";
								//$_SESSION['result_class']='error_response';
								//$_SESSION['result_message']="$message ";
							}
...
//this is for adding patient to waitng list2
elseif(isset($_POST['token_allocate1']) and isset($_SESSION['token_allocate1']) and $_POST['token_allocate1']==$_SESSION['token_allocate1']
	and userHasRole($pdo,48)){
		$_SESSION['allocate_surgery']  = $_POST['allocate_surgery'] ;
		$_SESSION['patient_type']= $_POST['patient_type'];
		$_SESSION['search_by']  = $_POST['search_by'];
		$_SESSION['search_ciretia'] = $_POST['search_ciretia'];
		$_SESSION['unregistered_patient'] = $_POST['unregistered_patient'];
		$_SESSION['expedite_reason_registered'] = $_POST['expedite_reason_registered'];
		$_SESSION['selected_patient'] = $_POST['selected_patient'] ;

		if(isset($_SESSION['allocate_surgery'])){
			$_SESSION['allocate_surgery']  = $_POST['allocate_surgery'] ;
			//echo "allocate_surgery" .$_SESSION['allocate_surgery'];
		}else{
			$_SESSION['allocate_surgery']  = '';
		}
		if(isset($_SESSION['patient_type'])){
			$_SESSION['patient_type']= $_POST['patient_type'];
		}else{
		...
		if(isset($_SESSION['search_ciretia'])){
			$_SESSION['search_ciretia'] = $_POST['search_ciretia'];
		}else{
			$_SESSION['search_ciretia']  = '';
		}
		if(isset($_SESSION['unregistered_patient'])){
			$_SESSION['unregistered_patient'] = $_POST['unregistered_patient'];
		}else{
			$_SESSION['unregistered_patient']  = '';
		}
	
		if(isset($_SESSION['expedite_reason_registered'])){
			$_SESSION['expedite_reason_registered'] = $_POST['expedite_reason_registered'];
		}else{
			$_SESSION['expedite_reason_registered']  = '';
		}
		if(isset($_SESSION['selected_patient'])){
			$_SESSION['selected_patient'] = $_POST['selected_patient'] ;
		}else{
			$_SESSION['selected_patient']  = '';
		}
	...
	if(isset( $_POST['search_ciretia']) and  $_POST['search_ciretia']!='' and $_POST['search_by'] !='patient_number' ){
	
			$_SESSION['selected_patient'] = $_POST['selected_patient'] ;		
			$result  = check_if_patient_exists($_POST['search_by'], $_POST['search_ciretia'],$pdo,$encrypt);
			
			//echo json_encode($both_pt) ;
			//echo json_encode($result) ;
			$searched_patient_pid = $_SESSION['searched_patient_pid'];
		// check if the patient is swapped
		if( isset($searched_patient_pid) and $result == 1 and $action =='' ){
			$resultx = check_if_swapped($pdo,'pid',$searched_patient_pid);
			$resultx2 = $search_id='';
			//echo $resultx;
			if($resultx !='good'){
				$exit_flag=true;
				$search_id =$searched_patient_pid;					

				while($resultx2 !='good'){
					$resultx2 = check_if_swapped_patient_number($pdo,'pid',$search_id);
					if($resultx2 !='good'){
						$search_id = $resultx2;
						//echo $search_id;
					}
					$pid=$encrypt->encrypt($search_id);
					
					$both_pt = array();
					$both_pt = check_both_religion_and_segment($pdo,$pid,$encrypt);
					//echo $both_pt;
					echo json_encode($both_pt) ;
					exit;
			...
			elseif($resultx =='good'){
				//$pid=$encrypt->encrypt($_SESSION['searched_patient_pid']);
				$pid=$encrypt->encrypt($searched_patient_pid);
				//$pid = $searched_patient_pid;
				//echo $pid;
				$both_pt = array();
				$both_pt = check_both_religion_and_segment($pdo,$pid,$encrypt);
				//echo $both_pt;
				...
				$message= "bad#No such patient";
				$_SESSION['result_class']='bad';
				//$_SESSION['result_message']="$message";
				$_SESSION['result_message']= $message;
				echo $message;
				echo $resultx;
				...
				//$message="bad#Please fill in this patient Segment under patient contact then retryxxxxxxx :";
			}
	...
	elseif(isset( $_POST['search_ciretia']) and  $_POST['search_ciretia']!='' and $_POST['search_by']=='patient_number' and userHasRole($pdo,48))
	{
		...
		if( isset($searched_patient_pid) and $result == 1 and $action =='' ){
			...
			// check for patient segment if filled
				
			

		// check for patient religion if set before the patient allocated
		if( isset($searched_patient_pid) and $result == 1 ){

				
			$patient_religion = '';
			$pid =  $searched_patient_pid;
			$patient_religion =check_selected_patient_religion($pdo,$pid);
			if(!$exit_flag AND ($patient_religion =='' ||  $patient_religion == NULL)){
				$exit_flag=true;
				$message="bad#Please fill in this patient Religion first under patient contact then retry :";
			}
	...
	if(!$exit_flag){		
		...
		if(!$exit_flag and  $result == 1){
			if( !$exit_flag and isset($searched_patient_pid) and $result == 1 and $action =='' ) {
				//$searched_patient_pid=$encrypt->decrypt($_POST['selected_patient']);	
				$resultx = check_if_swapped($pdo,'pid',$searched_patient_pid);
				$resultx2 ='';
				if($resultx !='good'){
					...
					//echo $search_id;
					$_SESSION['searched_patient_pid'] = $search_id;
					...
					//$exit_flag=true;
					//$message="bad#$resultx and cannot be edited2.";
					//echo $resultx2;
				}
			}
			...
			if(!$exit_flag and $_POST['patient_type']=='registered'){
				...
				// check for patient religion if filled
				...
				//now insert
					...
					if(!$exit_flag){
						try{
							...
							$tx_result = $pdo->commit();
							$record_found="submited but not the right allocation";
							echo  json_encode($record_found);
						}
						catch (PDOException $e)
						{
...
/////////////////////************************start add patient to waiting list************************/

elseif(isset($_GET['selected_patient'])  and ($_GET['action']== 'allocate_pt_record') and userHasRole($pdo,48) ){
	...
	$tplan_visit_id=0;
	$result='';
	//echo "selected_patient" .$_GET['selected_patient'];
	$allocate_surgery = $_SESSION['allocate_surgery'];
	//echo "allocate_surgery" .$allocate_surgery;
	$patient_type = $_SESSION['patient_type'];

	$search_by = $_SESSION['search_by'];
	$search_ciretia = $_SESSION['search_ciretia'];
	$unregistered_patient = $_SESSION['unregistered_patient'];
	$expedite_reason_registered = $_SESSION['expedite_reason_registered'];

			
			//check if selectefd  patient is set			

			if(!$exit_flag and  isset($_GET['selected_patient']) and $_GET['selected_patient']!=''){
				$searched_patient_pid=$encrypt->decrypt($_GET['selected_patient']);
				$result=1;
				//echo "kk";
			}			
			...
			if(!$exit_flag and (!isset($allocate_surgery) or $allocate_surgery=='')){
				$exit_flag=true;
				$message="bad#Please select the surgery to allocate the patient to.";			
			}
			//check patient type is set
			if(!$exit_flag and $patient_type =='' and $patient_type!='registered' and $patient_type!='unregistered'){
				$exit_flag=true;
				$message="bad#Please select the patient type";			
			}
			if(!$exit_flag and $patient_type =='registered' and (!isset($patient_type) or $patient_type =='')){
				$result  = check_if_patient_exists($search_by, $search_ciretia,$pdo,$encrypt);
				echo $result;
				$data = explode('#',$result);
				$result=$data[0];
				if(isset($data[1])){$searched_patient_pid=$data[1];}
			...
			if( isset($searched_patient_pid) ){

					
					$patient_segment_in = '';
					$pid =  $searched_patient_pid;
					$patient_segment_in =check_selected_patient_segment($pdo,$pid);
					if(!$exit_flag AND ($patient_segment_in =='' ||  $patient_segment_in == NULL)){
						$exit_flag=true;
						$message="bad#Please fill in this patient Segment under patient contact then retry :";
					}
			...
			//check if unregisterd is selected and patient selected
			if(!$exit_flag and  $patient_type =='unregistered' and isset($unregistered_patient) and $unregistered_patient==''){
				$exit_flag=true;
				$message="bad#Please select the unregistered patient to allocate";		
			}	
			if(!$exit_flag and $patient_type =='unregistered' and $unregistered_patient !=''){
				$result=1;
			}	
	if(!$exit_flag){		
		//if one patient is found then do submit
		if(!$exit_flag and  $result == 1){
			//check if this patient is already allocated and has not been discharged or has not left
			if(!$exit_flag and $patient_type =='registered'){
				$sql=$error=$s='';$placeholders=array();
				$sql="select pid from patient_allocations where pid=:pid and patient_type=1 and date(time_allocated)=curdate() and 
						date(discharge_time)='0000-00-00'";
				$error="Unable to check if patient is already allocated";
			...
			//check if this patient is already allocated and has not been discharged or has not left
			if(!$exit_flag and $patient_type =='unregistered'){
				$sql=$error=$s='';$placeholders=array();
				$sql="select pid from patient_allocations where pid=:pid and patient_type=0 and date(time_allocated)=curdate() and 
						date(discharge_time)='0000-00-00'";
				$error="Unable to check if patient is already allocated";
				$placeholders[':pid']=$encrypt->decrypt($unregistered_patient);
				$s = 	select_sql($sql, $placeholders, $error, $pdo);
				if($s->rowCount() > 0){
					$exit_flag=true;
					$message="bad#That  patient has already been allocated.";
					...
					if(!$exit_flag){
						try{
							...
							$sql="insert into patient_allocations set surgery_id=:surgery_id, pid=:pid, time_allocated=current_timestamp(), 
								patient_type=:patient_type,		 points_per_min=:points_per_min,tplan_visit_id=:tplan_visit_id,visit=:visit,added_by=:added_by, expedite=:expedite";
							$error="Unable to get referrer name";
							$placeholders[':surgery_id']=$encrypt->decrypt($allocate_surgery);
							$placeholders[':points_per_min']=$points_per_minute;
							$placeholders[':tplan_visit_id']=$tplan_visit_id;
							$placeholders[':visit']="$visits";
							...
							if($patient_type =='registered'){
								$placeholders[':pid']=$searched_patient_pid;
								$placeholders[':expedite']=$expedite_reason_registered;
							}
							elseif($patient_type =='unregistered'){
								$placeholders[':pid']=$encrypt->decrypt(unregistered_patient);
								$placeholders[':expedite']=$expedite_reason_unregistered;
							}
							//check patient type
							if($patient_type =='registered'){$placeholders[':patient_type']=1;}
							elseif($patient_type =='unregistered'){$placeholders[':patient_type']=0;}
							$s = 	select_sql($sql, $placeholders, $error, $pdo);	
							if($s){
								$message='good#patient_allocation#Patient allocated';
								//calculate balance if not set before
							...
							//$record_found="nothing";
							$tx_result = $pdo->commit();
							$record_found="";
							
							if($tx_result){
								$message="good#patient_allocation#Patient allocated";
								$record_found="updated sawa";
							}

							if(!$tx_result){
								$message="bad#Unable to allocate patient";
								$record_found="not allocated";
								echo  json_encode($record_found) ;
							}							
		...
		//echo "<meta http-equiv=\"refresh\" content=\"0;URL=http://localhost/?id=allocate-patients\">";
		echo $message;	
...
//////**************************end add patient to waiting list***************************************** */
		...
		if(!$exit_flag){	
			try{
				...
				if($pay_type == 2 or $pay_type == 3  or $pay_type == 4  or $pay_type == 5  or 
					...
					$s = insert_sql($sql, $placeholders, $error, $pdo);
					if($_SESSION['branch']==1){$receipt_number="LR$rid-".date('m/y');}		
					elseif($_SESSION['branch']==0){$receipt_number="R$rid-".date('m/y');}
					$receipt_num_id=$rid;
					...
					if($receipt_number != ''){
						...
						$placeholders[':branch']=$_SESSION['branch'];
						$placeholders[':receipt_num']="$receipt_number";
	...
	//		echo "-3195-";
			...
			if($pay_type == 7){//this will generate an invoice number for the xray
				...
				$invoice_number="X$xinv-".date("m/y");*/
							...
							$placeholders[':xinv']=$xinv;
							$s = insert_sql($sql, $placeholders, $error, $pdo);
							if($_SESSION['branch'] ==1){$invoice_number="LX$xinv-".date("m/y");}
							elseif($_SESSION['branch'] ==0){$invoice_number="X$xinv-".date("m/y");}
				...
				$sql="insert into unique_invoice_number_generator set invoice_number=:invoice_number, when_raised=now(),
					added_by=:added_by, pid=:pid, branch=:branch";
				$placeholders[':invoice_number']="$invoice_number";
				$placeholders[':added_by']=$_SESSION['id'];
				$placeholders[':branch']=$_SESSION['branch'];
				$placeholders[':pid']=$pid;
		...
		//	else{$authorised_cost=$amount;}
			...
			//		echo "<br>$empty-$count-$nimeana";
				...
				if($pay_type == 7){					
					...
					$error2="Unable to add xrays to tplan";
					$placeholders2[':tplan_id']=$tplan_id;
					$placeholders2[':branch']=$_SESSION['branch'];
					$placeholders2[':unathorised_cost']=$amount;
					$placeholders2[':authorised_cost']=$authorised_cost;
					$placeholders2[':invoice_number']="$invoice_number";
					$placeholders2[':pid']=$pid;
			...
			if($pay_type == 7){			
				...
				foreach($s as $row){
					...
					if($deduction!=''){
						...
						$placeholders2[':invoice_number']=$inv_num;
						$s2 = insert_sql($sql2, $placeholders2, $error2, $pdo);	
				...
				//	echo "invoice is $_POST[$raise_invoice] ";
					...
					if($invoice_type=="new" and $existing_new==''){//raise new invoice number
						...
						if($_POST["$append_invoice"]==''){
							...
							$placeholders[':inv_id']=$inv_id;
							$s = insert_sql($sql, $placeholders, $error, $pdo);	
							if($_SESSION['branch'] == 1){$invoice_num="LI$inv_id-".date("m/y");}							
							else{$invoice_num="I$inv_id-".date("m/y");}
							$existing_new="$invoice_num";
							...
							$sql2=$error2=$s2='';$placeholders2=array(); 
							$sql2="insert into unique_invoice_number_generator set pid=:pid, invoice_number=:invoice_number,
								when_raised=now(),added_by=:added_by, branch=:branch";
								$placeholders2[':pid']=$_SESSION['pid'];
								$placeholders2[':branch']=$_SESSION['branch'];
								$placeholders2[':invoice_number']="$invoice_num";
								$placeholders2[':added_by']=$_SESSION['id'];
					...
					$sql=$error=$s='';$placeholders=array();
					$sql="update tplan_procedure set invoice_number=:invoice_number, date_invoiced=now(), invoice_id=:invoice_id, branch=:branch where treatment_procedure_id=:treatment_procedure_id";
					$error="Unable to update treatment procedure invoice number";
					$placeholders[':invoice_number']="$invoice_num";
					$placeholders[':invoice_id']=$invoice_id;
					$placeholders[':treatment_procedure_id']="$treatment_procedure_id";
				...
				if(isset($_POST["$raise_quotation"]) and $_POST["$raise_quotation"]!=''){
					...
					if($quotation_type=="new" and $existing_quote_new==''){//raise new quotation number
						...
						//echo "z2 $_POST[$append_quotation] ";
							...
							if($_SESSION['branch'] == 1){$quotation_num="LQ$quoataion_id-".date("m/y");}
							else{$quotation_num="Q$quoataion_id-".date("m/y");}
					...
					$sql=$error=$s='';$placeholders=array();
					$sql="update tplan_procedure set quotation_number=:quotation_number, branch=:branch where treatment_procedure_id=:treatment_procedure_id";
...
//this will create scholl holiday appointment from t.done
elseif( isset($_POST['token_shr']) and 	isset($_SESSION['token_shr']) and 	$_POST['token_shr']==$_SESSION['token_shr'] ){
	$exit_flag=false;
	
	//check if holiday  is set
	if(!isset($_POST['holiday_period']) or $_POST['holiday_period']==''){
		$exit_flag=true;
		$message="bad#Please specify the   for the holiday period";
	}

	//check if task  is set
	if(!$exit_flag and !isset($_POST['holiday_task']) or $_POST['holiday_task']==''){
		$exit_flag=true;
		$message="bad#Please specify the task";
	}	
	...
	if(!$exit_flag  ){  
			...
			$sql="insert into school_holiday_appointment_reminders set when_added=now(), 
					...
					added_by=:added_by";
			$error="Unable to get add appointment";
			$placeholders[':doc_id']=$encrypt->decrypt($_POST['token_ninye2']);
			$placeholders[':pid']=$encrypt->decrypt($_POST['token_ninye']);
			$placeholders[':added_by']=$_SESSION['id'];
...
elseif(isset($_POST['tdone_appointment_date5']) and $_POST['tdone_appointment_date5']!='' and userHasRole($pdo,122)){
	...
	//get appointments on that day first for registerd folks
	$sql=$error=$s='';$placeholders=array();
	$sql="select a.last_name,a.middle_name, a.first_name,b.first_name,b.middle_name,b.last_name,c.treatment,c.shour,c.smin,c.rank,c.surgical_unit,c.am_pm,c.duration,c.priority
	from patient_details_a a, users b, registered_patient_appointments c where c.pid=a.pid and c.doc_id=b.id and c.appointment_date=:appointment_date and c.status='NOT SEEN'";
	$placeholders[':appointment_date']=$appointment_date;
	$error="Unable to get registerd appointments";
	$s = 	select_sql($sql, $placeholders, $error, $pdo);
	foreach($s as $row){
		...
		$doctor_name=html("$row[3] $row[4] $row[5]");
		$treatment='';
		if ($row['treatment'] != ''){$treatment="Treatment: ".html("$row[treatment]");}
		$hour=html("$row[shour]");
		$min=html("$row[smin]");
		$rank=html("$row[rank]");
		$surgery=html("$row[surgical_unit]");
		$duration=html("$row[duration]");
		$priority='';
		if ($row['priority'] == 1){$priority="<br>Priority: Low";}
		elseif ($row['priority'] == 2){$priority="<br>Priority: Medium";}
		elseif ($row['priority'] == 3){$priority="<br>Priority: High";}
		$appointments[]=array('registered'=>'yes','patient_name'=>"$patient_name", 'doctor_name'=>"$doctor_name",'hour'=>"$hour",'min'=>"$min",'rank'=>"$rank",'surgery'=>"$surgery",'treatment'=>"$treatment",'duration'=>"$duration",'priority'=>"$priority");
	}
	
	//get appointments on that day first for unregisterd folks
	$sql=$error=$s='';$placeholders=array();
	$sql="select concat(a.first_name,' ',a.middle_name,' ',a.last_name) as names,b.first_name,b.middle_name,b.last_name,c.treatment,c.shour,c.smin,c.rank,c.surgical_unit,c.am_pm,c.duration,c.priority
	from unregistered_patients a, users b, unregistered_patient_appointments c where c.pid=a.id and c.doc_id=b.id and c.appointment_date=:appointment_date and c.status='NOT SEEN'";
	$placeholders[':appointment_date']=$appointment_date;
	$error="Unable to get un-registerd appointments";
	$s = 	select_sql($sql, $placeholders, $error, $pdo);
	foreach($s as $row){
		...
		elseif ($row['priority'] == 2){$priority="<br>Priority: Medium";}
		elseif ($row['priority'] == 3){$priority="<br>Priority: High";}
		$appointments[]=array('registered'=>'no','patient_name'=>"$patient_name", 'doctor_name'=>"$doctor_name",'hour'=>"$hour",'min'=>"$min",'rank'=>"$rank",'surgery'=>"$surgery",'treatment'=>"$treatment",'duration'=>"$duration",'priority'=>"$priority");
	}
	...
	foreach($s2 as $row){
		...
		//	echo "<tr><td class='headcol'>$hour:$minute $am_pm</td>";
			...
			$i2=0;
			while($i2 < $n2){
				if(isset($rowspan_array[$i2]) and $rowspan_array[$i2] > 1){
					$rowspan_array[$i2]--;
					$i2++;
					continue;
				}
				...
				foreach($appointments as $current_appointment){
					if($surgery_id_array[$i2] == $current_appointment['surgery'] and $current_appointment['rank'] == $rank and 
						...
						//check if treatment is defined and show this
						$task='';
						//skip nuumber of time slots based on appointment duration
						if($current_appointment['duration'] > 0){
							$rowspan_array[$i2] = $current_appointment['duration']  ;
						}
						else{$rowspan_array[$i2]=1;}	
						if($current_appointment['treatment']!=''){$task="<br>$current_appointment[treatment]";}
						echo "<td rowspan=$rowspan_array[$i2] class=$td_class>$current_appointment[patient_name] $hour:$minute $am_pm $task 
						   $current_appointment[priority] 
						</td>";
						}
				}
				if(!$appointment_exists){
					echo "<td>";
						...
						echo "<input type=button value='$hour:$minute $am_pm $surgery_name_array[$i2] ' class=' button_style toggle_appointment_form' />";
							...
							//get number of slots before next appointment
							$slots = get_time_slots($pdo,"$appointment_date", "$rank", "$minute", "$surgery_id_array[$i2]");
							echo "<label class=label>Duration</label>";
							echo "<select name=appointment_duration><option></option>";
							$i3=1;
							while ($i3 <= $slots){
								echo "<option value=$i3>$time_slot_array[$i3]</option>";
								$i3++;
							}
							echo "	</select><br><br>";
							
							echo "<label class=label>Priority</label>";
							echo "<select name=appointment_priority><option></option>
			...
			while($i2 < $n2){
				...
				foreach($appointments as $current_appointment){
					if($surgery_id_array[$i2] == $current_appointment['surgery'] and $current_appointment['rank'] == $rank and 
						...
						echo "<td rowspan=$rowspan_array[$i2] class=$td_class>$current_appointment[patient_name] $hour:$minute $am_pm $task 
						   $current_appointment[priority] </td>";
						}
				}
				if(!$appointment_exists){
...
$_POST['token_search_patient_appoint3']==$_SESSION['token_search_patient_appoint3'] and userHasRole($pdo,45)){
	...
	if(!$exit_flag and $_POST['appointment_duration']==''){
		$exit_flag=true;
		$message="bad#Unable to save appointments, missing appointment duration.";
	}
	
	//check if appointment_treatment is set
	if(!$exit_flag and $_POST['appointment_treatment']==''){
		$exit_flag=true;
		$message="bad#Unable to save appointments, missing treatment to be done.";
	}
	
	//check if priority is set
	if(!$exit_flag and $_POST['appointment_priority']==''){
		$exit_flag=true;
		$message="bad#Unable to save appointments, missing appointment priority.";
	}
	...
	if(!$exit_flag and $result==1){  
		if( $_POST['patient_type']=='registered' and $searched_patient_pid!=''){
			...
			$placeholders[':surgical_unit']=$surgical_unit;
			$placeholders[':duration']=$_POST['appointment_duration'];
			$placeholders[':priority']=$_POST['appointment_priority'];
			$placeholders[':treatment']=$_POST['appointment_treatment'];
			$s = 	select_sql($sql, $placeholders, $error, $pdo);	
	...
	/*else{
		...
		if($data[0]=='good'){
			$_SESSION['token_search_patient_appoint3']='';
			$_SESSION['result_class']='success_response';
			$_SESSION['result_message']="$data[2]";
		}
	echo "$message";
...
elseif(isset($_POST['create_appointment']) and $_POST['create_appointment']!='' and userHasRole($pdo,45) ){
	...
	<div id=registered_appointment>
			...
			<form class='patient_form check_selected_patient' action="" method="post" name="" id="">
				...
				echo "<div class='prefix-5 grid-10 label'>Duration</div><div class='grid-15 label'>";
					echo "<select name=appointment_duration><option></option>";
						...
						while ($i3 <= $slots){
							...
							<option value=4>  1 hour</b></option>
							...
							<option value=9>2 hrs 15 min</option>
							<option value=10>2 hrs 30 min</option>
							<option value=11>2 hrs 45 min</option>
							<option value=12>3 hours</option>*/
					echo "	</select>
					</div><div class=clear></div></br>";
				
				echo "<div class='grid-20 label'>Treatment</div>";
				echo "<div class='grid-25'><input type=text name=appointment_treatment /></div>";			
				echo "<div class='prefix-30 grid-10 label'>Priority</div><div class='grid-15 label'>";
					echo "<select name=appointment_priority><option></option>
							<option value='1'>Low</option>
							<option value='2'>Medium</option>
							<option value='3'>High</option>
				...
				<div class='prefix-20 grid-15'><input class='' type=submit value='Book Appointment' /></div></form><?php
	echo "</div>";//this is for registered_appointment div
	echo "<div id=unregistered_appointmentss></div>";
...
elseif(isset($_SESSION['token_tdone_appointment']) and isset($_POST['token_tdone_appointment']) and $_SESSION['token_tdone_appointment']==$_POST['token_tdone_appointment']){
	$exit_flag=false;
	if(!isset($_POST['appointment_duration']) or $_POST['appointment_duration'] == ''){
		$exit_flag=true;
		$message = "bad#Unable to save appointment as appointment duration not set";
	}
	if(!isset($_POST['appointment_priority']) or $_POST['appointment_priority'] == ''){
		$exit_flag=true;
		$message = "bad#Unable to save appointment as appointment priority not set";
	}
	if(!isset($_POST['appointment_task']) or $_POST['appointment_task'] == ''){
		$exit_flag=true;
		$message = "bad#Unable to save appointment as appointment treatment not set";
	}
	if(!$exit_flag){
		$data1=$encrypt->decrypt($_POST['ninye']);
		$data=explode('#',"$data1");
...
elseif(isset($_POST['edit_tplan']) and $_POST['edit_tplan']!='' and userHasRole($pdo,51)){
	...
	$self_sum=number_format($self_sum,2);
	 $_SESSION['procedure_encryp_id_array']= array();
		...
		$n=count($treatment_procedure_id_array);
		$i=$count=$old_count=0;
		// edit procedure upgrade ---by dan winga
			...
			elseif(!$invoiced_array[$i]){
				...
				elseif("$status_array[$i]" == 'Not Started'){
					...
					echo "<tr><td>$count</td><td>";
						...
						$error2="Unable to get prodcedures";
						$s2 = 	select_sql($sql2, $placeholders2, $error2, $pdo);	
						//echo 'testingdhdhhdhdddd";--------DAN WINGA';
						//echo "<input class='input_in_table_cell codex'  type=text name=codex$old_count id=$old_count  placeholder= 'Click to add Procedure Name'  />";
						echo "<select name=old_procedure$old_count class='input_in_table_cell select_procedure21' ><option></option>";
						 $sub_array = array();
							foreach($s2 as $row2){
								$val2=$encrypt->encrypt(html($row2['id']));	
								$sub_array['id'] = html($row2['id']);
							...
							echo "</select>";							
							//echo "<input class='input_in_table_cell select_procedure2'  type=hidden name=procedure$old_count id=procedure$old_count  placeholder= ' add Procedure value'   />";
							$myOutPut;							
							$_SESSION['procedure_encryp_id_array']=$myOutPut;
						//show teeth
						//if($all_teeth_array[$i] == 'yes'){
				...
				<?php	//	} //end show teeth
					...
					$cash_pay=$encrypt->encrypt("2");
					$points_pay=$encrypt->encrypt("3");
					echo "</td><td><textarea  class=tplan_details  rows='' name=old_details$old_count id=details$old_count >$details_array[$i]</textarea></td><td>
						<select  name=old_pay_method$old_count id=pay_method$old_count class='input_in_table_cell pay_method' ><option></option>";
						if($insured == 'YES'){echo "<option value='$invoice_pay' $insurance_selected >Insurance</option>";}		
							echo "<option value='$cash_pay' $self_selected >Self</option>
							<option value='$points_pay' $points_selected >Points</option>
						</select></td><td><input type=text name=old_cost$old_count id=cost$old_count value=$unauthorised_cost_array[$i] class=tplan_cost2 />";
						if(userHasRole($pdo,113)){
							$alias_val=$encrypt->encrypt("ta$i");
							echo "Aliased <input  class=tplan_alias type=checkbox name=old_alias$old_count id=alias$old_count  value=$alias_val $checked_alias />";
						}
						echo "</td><td>N/A</td>
					<td><input type=checkbox name=old_remove$old_count id=old_remove$old_count value=$treatment_procedure_id_array[$i] class=tplan_remove />
						<input type=hidden name=old_ninye$old_count id=old_ninye$old_count value=$treatment_procedure_id_array[$i] class=tplan_remove />
					</td></tr>";
				}
			}
			$i++;
...
elseif(isset($_SESSION['token_a1_patinet']) and isset($_POST['token_a1_patinet']) and $_POST['token_a1_patinet']==$_SESSION['token_a1_patinet']){
	...
	//perform verifications
	$exit_flag=false;
	$photo_path=$other_relationship=$ptype=$company_covered=$pnum=$email_address=$email_address_2=$year=$city=$weight=$referee=$em_relationship=$religion=$patient_segment=$patient_source='';
...
//	echo "38";
	...
	//check Religion  for on behalf form filling
	$religion ='';
	if(!$exit_flag and isset($_POST['religion']) and $_POST['religion']!=''){
		$religion=html($encrypt->decrypt($_POST['religion']));
		
		
		if(!in_array($religion,$_SESSION['religion_array'])){
			
			$exit_flag=true;
			$message="somebody tried to input $religion as a on behalf religion into patient details";
			log_security($pdo,$message);
			$message="bad#Unable to save details as religion for form filler  is not correctly specified. ";
		}
			
	}

	//check Patient segment on behalf  of form filling
	$patient_segment ='';
	if(!$exit_flag and isset($_POST['patient_segment']) and $_POST['patient_segment']!=''){
		$patient_segment=html($encrypt->decrypt($_POST['patient_segment']));
		
		
		if(!in_array($patient_segment,$_SESSION['patient_segment_array'])){
			
			$exit_flag=true;
			$message="somebody tried to input $patient_segment as  on behalf patient Segment into patient details";
			log_security($pdo,$message);
			$message="bad#Unable to save details as patient segment, for form filler  is not correctly specified. ";
		}
			
	}

	//check patient source
	$patient_source='';
	if(!$exit_flag and isset($_POST['patient_source']) and $_POST['patient_source']!=''){
		$patient_source=html($encrypt->decrypt($_POST['patient_source']));
		
		
		if(!in_array($patient_source,$_SESSION['patient_source_array'])){
			
			$exit_flag=true;
			$message="somebody tried to input $patient_source as  on behalf patient Source into patient details";
			log_security($pdo,$message);
			$message="bad#Unable to save details as patient source, for form filler  is not correctly specified. ";
		}
	...
	//check  religion
	if(!$exit_flag and $_POST['religion']=='' and $_POST['religion']==''){
		
		$exit_flag=true;
			$message="bad#Unable to save details as religion  is not specified ";
		
	}

	//check  patient segment
	if(!$exit_flag and $_POST['patient_segment']=='' and $_POST['patient_segment']==''){
		
		$exit_flag=true;
			$message="bad#Unable to save details as patient segment  is not specified ";
	...
	if(!$exit_flag and (!isset($_SESSION['pid']) or $_SESSION['pid']=='')){
		...
		//echo "insert";exit;
			...
			$sql="insert into patient_details_a set last_name=:last_name, middle_name=:middle_name, first_name=:first_name, mobile_phone=:mobile_phone,
					...
					year=:year,email_address=:email_address, email_address_2=:email_address_2, card_issued=:card_issued, added_by=:added_by";
			$error="Unable to add patient new patient";
			$placeholders[':last_name']=$_POST['last_name'];
			$placeholders[':middle_name']=$_POST['middle_name'];
			$placeholders[':first_name']=$_POST['first_name'];
			...
			$sql="insert into patient_details_b set id_number=:id_number, address=:address, city=:city, occupation=:occupation,
					...
					gender=:gender,	photo_path=:photo_path, pid=:pid, weight=:weight, dob=:dob, referee=:referee, em_contact=:em_contact, religion=:religion ,patient_segment=:patient_segment,
			...
			$placeholders[':religion']=$religion;
			$placeholders[':patient_segment']=$patient_segment;
			$placeholders[':patient_source']=$patient_source;
			$placeholders[':pid']=$id;
	...
	elseif(!$exit_flag and (isset($_SESSION['pid']) and $_SESSION['pid']!='')){
		try{
			...
			$sql="update patient_details_a set last_name=:last_name, middle_name=:middle_name, first_name=:first_name, mobile_phone=:mobile_phone,
					...
					email_address=:email_address, email_address_2=:email_address_2, card_issued=:card_issued where pid=:pid";
			...
			$sql="update patient_details_b set id_number=:id_number, address=:address, city=:city, occupation=:occupation,
					...
					gender=:gender,	photo_path=:photo_path, weight=:weight, dob=:dob, referee=:referee, em_contact=:em_contact, religion=:religion , 
					patient_segment=:patient_segment , patient_source=:patient_source where pid=:pid";
			$error="Unable to update patient details";
			$placeholders[':id_number']=$_POST['id_no'];
			$placeholders[':address']=$_POST['address'];
			$placeholders[':city']=$city;
...
// update segment and religion  for old client while allocating patient

//if( (isset($_GET['selected_upd']) and $_GET['selected_upd']!='') and (isset($_GET['action']) and $_GET['action'] =='update_pt_record' ) ){
if( (isset($_GET['selected_upd']) and (isset($_GET['action']) and $_GET['action'] =='update_pt_record' ) and $_GET['selected_upd']!='') and (isset($_GET['religion_upd']) and $_GET['religion_upd']!='') and (isset($_GET['segment_upd']) and $_GET['segment_upd']!='' )){
	try{
		//$patient_segment = html($encrypt->decrypt($_GET['segment_upd']));
		//$religion = html($encrypt->decrypt($_GET['religion_upd']));
		$udpid = html($encrypt->decrypt($_GET['selected_upd']));
		$religion = html($_GET['religion_upd']);
		$patient_segment = html($_GET['segment_upd']);
		$record_found = "iiiiidddddddddddddddd";
		$pdo->beginTransaction();				
					
		//now update patient_details_b
		$sql=$error=$s='';$placeholders=array();
		$sql="update patient_details_b set  religion=:religion , 
				patient_segment=:segment where pid=:pid";
		$error="Unable to update patient details";
		
		$placeholders[':religion']=$religion;
		$placeholders[':segment']=$patient_segment;
		$placeholders[':pid']=$udpid;
		$s = 	insert_sql($sql, $placeholders, $error, $pdo);	
		//$pdo->commit();		
		$record_found = "after record insert---update";
		
		if(!$exit_flag){$tx_result = $pdo->commit();
			$record_found="updated ok";
		}
		elseif($exit_flag){
			//delete photo if set
	...
	{
	$pdo->rollBack();
	echo $message="bad#   Unable to save patient details  ";
	}	
}


//update patient insurance
if( (isset($_GET['selected_updxxxx']) and (isset($_GET['actionxxx']) and $_GET['action'] =='update_pt_insurance_card_bal' ) and $_GET['selected_updxxxx']!='') and (isset($_GET['card_bl_upd']) and $_GET['card_bl_updccc']!='')){
	$udpid = html($encrypt->decrypt($_GET['selected_upd']));
	$card_bal = floatval (html($_GET['card_bl_upd'])); 
	$record_found =  $udpid;
	$record_found2= $card_bal ;
	$record_found3 = $record_found ."and :". $record_found2;
	echo  json_encode($record_found3) ;

}

if( (isset($_GET['selected_upd']) and (isset($_GET['action']) and $_GET['action'] =='update_pt_insurance_card_bal' ) and $_GET['selected_upd']!='') and (isset($_GET['card_bl_upd']) and $_GET['card_bl_upd']!='')){
	try{
		
		$udpid = html($encrypt->decrypt($_GET['selected_upd']));
		$card_bal = floatval (html($_GET['card_bl_upd']));
		$record_found3 = $card_bal ."and :". $udpid;
		$timestamp = date("Y-m-d H:i:s");
		$pdo->beginTransaction();

		
		//check if there is a similer record 
		$sql=$error=$s='';$placeholders=array();
		$sql="select * from patient_card_status where pid=:pid";
		$error="Unable to select patient card status";
		$placeholders[':pid']=$udpid;
		$s = 	select_sql($sql, $placeholders, $error, $pdo);
		
		if($s->rowCount() > 0){
			
			//now update patient card status 
				
				$sql2=$error2=$s2='';$placeholders2=array();
				$sql2="update patient_card_status set  card_amount=:card_amount , check_date=:check_date where pid=:pid";
				$error2="Unable to select patient card status";
				
				$placeholders2[':card_amount']=$card_bal;
				$placeholders2[':check_date']=$timestamp ;
				$placeholders2[':pid']=$udpid;
				$s2 = 	insert_sql($sql2, $placeholders2, $error2, $pdo);	
				$record_found = "after record insert---update";
		
				if(!$exit_flag){$tx_result = $pdo->commit();
					$record_found="card insurance balance updated ok";
				}
				elseif($exit_flag){
					//delete photo if set
			...
			//now insert new company
			$sql3=$error3=$s3='';$placeholders3=array();
			$sql3="insert into patient_card_status set  pid=:pid, card_amount=:card_amount, check_date=:check_date ";
			$error3="Unable to insert patient_card_status";
			$placeholders3[':card_amount']=$card_bal;
			$placeholders3[':check_date']=$timestamp ;
			$placeholders3[':pid']=$udpid;
						
			$s3 = 	insert_sql($sql3, $placeholders3, $error3, $pdo);	
			
			if(!$exit_flag){$tx_result = $pdo->commit();
				$record_found="insurance balance added ok";
			}
			elseif($exit_flag){
			
				$tx_result=false;$pdo->rollBack();				
				$error3="Unable to add new patient_card_status";
	...
	{
	$pdo->rollBack();
	echo $message="bad#   Unable to update patient patient_card_status  ";
	}	
}


if( isset($_GET['selected_upd']) and (isset($_GET['action']) and $_GET['action'] =='check_upd_action') ){
	$pid = $_GET['selected_upd']; 
	
		
		$searched_patient_pid = $_GET['selected_upd'];
		//$_SESSION['searched_patient_pid'] = $searched_patient_pid ;
		// check if the patient is swapped
		if( isset($searched_patient_pid) ){
			$resultx = check_if_swapped($pdo,'pid',$searched_patient_pid);
			$resultx2 = $search_id='';
	...
	//$record_found = check_both_religion_and_segment($pdo,$pid,$encrypt);
		//$record_found =check_selected_patient_segment($pdo,$pid);
	//echo json_encode(array_values($record_found));

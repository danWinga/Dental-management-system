...
//this is for adding patient to waitng list
//elseif(  isset($_POST['token_allocate1'])){
//$allocate_surgery =$patient_type = $search_by = $search_ciretia = $unregistered_patient = $expedite_reason_registered = '';

elseif(isset($_POST['token_allocate1']) and isset($_SESSION['token_allocate1']) and ($_POST['token_allocate1']== $_SESSION['token_allocate1']) and userHasRole($pdo,48)){
	$exit_flag=false;
	$tplan_visit_id=0;
	$result='';
	...
	if(isset( $_POST['search_ciretia']) and  $_POST['search_ciretia']!='' and $_POST['search_by']=='patient_number' and userHasRole($pdo,48)){
		...
		//echo $both_pt	;		
		//echo $patient_segment_in ;
		echo json_encode($both_pt) ;
		//echo json_encode(array_values($both_pt));
		
		

	}

	elseif(isset( $_POST['search_ciretia']) and  $_POST['search_ciretia']!='' and $_POST['search_by'] !='patient_number' ){
	...
	elseif(!$exit_flag and  isset($_POST['selected_patientxx']) and $_POST['selected_patientxx']!=''){
...
elseif( isset($_GET['selected_upd']) and (isset($_GET['action']) and $_GET['action'] =='check_upd_action') ){
...
//action:action, selected_upd:selected_upd	
elseif(isset($_GET['selected_patientxx'])  and ($_GET['actionxx']== 'allocate_pt_recordxx')) {
	$res = "allocating patient test okay".$_GET['selected_patient'];
	echo $res;
}
...
elseif(isset($_SESSION['token_a1_patinet']) and isset($_POST['token_a1_patinet']) and $_POST['token_a1_patinet']==$_SESSION['token_a1_patinet']){
	...
	//perform verifications
	$exit_flag=false;
	$photo_path=$other_relationship=$ptype=$company_covered=$pnum=$email_address=$email_address_2=$year=$city=$weight=$referee=$em_relationship=$religion=$patient_segment='';
	...
	elseif(!$exit_flag and (isset($_SESSION['pid']) and $_SESSION['pid']!='')){
		try{
			...
			$sql="update patient_details_b set id_number=:id_number, address=:address, city=:city, occupation=:occupation,
					...
					patient_segment=:patient_segment where pid=:pid";
			$error="Unable to update patient details";
			$placeholders[':id_number']=$_POST['id_no'];
			$placeholders[':address']=$_POST['address'];
			$placeholders[':city']=$city;

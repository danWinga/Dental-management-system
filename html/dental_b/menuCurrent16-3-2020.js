...
function trview(){
	...
	//$('#patient_form search_patient_2_submit').click(function() {
	$('#patient_form search_patient_2x').submit(function(e){
		e.preventDefault();
		alert('form was submit clicked ') ;
		var actionx ='submit_clicked';
		var xform  = $(this).serialize();
		
		$.ajax({
			method: "POST", 
			url: "patient-contacts/patient_other_contact.php",			
			data:{ actionx:actionx ,xform : xform } ,
			success: function(response){								
				//var recordFound = response;
				alert('form was submitted ' + response) ;
				//var data = JSON.parse( recordFound);
									
				//alert('found patient data is nnnnnn  ' + recordFound);
  ...
  function fill_treeview3(treeData)
   {
	...
	{	
		...
		//alert("response  id data  :"+ currentId +" ==Parent:"+ parent + "#procedure:"+codex_id) ;
		if (parent > 0){
...
//$(document.body).on('submit', '.patient_form', function(e){
	if($(this).hasClass('search_patient_2')){
		//alert ('yyyyyyyyyyyyyyyyyyyyyyyyyy');
	}
...
//	else{alert ('nnnnnnnnnnnnnnnnnnnnnn');}
	...
	//alert('data is type of  ' + x2);
		//	if(typeof x2[1] === 'undefined'){}
		//tukondani
		//var xc = data.split('#');
		
		if (data.length > 2 && data.length < 1000 ){
		...
		if(pid_pt !='' && (religion !='' || segment !='') ){
			//alert('data is  infor: pid: religion:'+ religion + ': segment :'+ segment  );
		...
		//alert('patient is ready for allocation----NNNNNNNNNNNNN---' );
				...
				$('.selected_pt').click(function() {
					...
					if(selected_pt !==''){
						...
						$.ajax({
							...
							if(pid_pt !='' && (religion !='' || segment !='') ){
								//alert('data is  infor: pid: religion:'+ religion + ': segment :'+ segment  );
								//alert('patient is ready for allocation-------' );
								var udpId = pid_pt;  
								allocate_patient_to_waiting_list(udpId);
								//$('.div_shower').dialog("close");
							...
							//alert('patient is ready for allocation----NNNNNNNNNNNNN---' );
							//alert('data is  infor: pid ' + pid_pt + ': religion:'+ religion + ': segment :'+ segment  );
							else if( pid_pt !='' && (segment =='' || religion =='')){
								var width_x = $('.get_width').show().width();
								element.append('<input type=hidden name=selected_patient class=selected_patient_input value=' + selected_pt + ' />');
								//element.submit();
								$('.div_shower').dialog("close");
							...
							//alert('found patient record are: idp : '+ idp + " segment: "+ segment + " religion :"+ religion );
							//location.reload();
							//element.submit();
							//window.location = "?id=allocate-patients";// id=allocate-patients
...
// selected_pt on click



//fill_missing_pt_contact data
function fill_missing_pt_contact(pid_pt){
		...
		$.ajax({
			...
			success: function(response){
				...
				fill_patient_dg(fname,mdname,lname,gender);
			...
			if(segmentx !='' && religionx !=''){
				...
				$.ajax({
					...
					success: function(response){
						...
						resStatus = true;	
						//alert('record ikosawa ama finally kkkkkkkkkkkkkkkkkkkkkkkkk: '+ response);
						//alert('record res1: '+ resStatus);
						//alert('record pid : '+ selected_pt);
						...
						window.resState =true;
							
						
						//alert('record ikosawa res Status: '+ resState);
...
resState = false;
function close_and_refresh(resState){
	//alert('checking if close_and refresh has been called');
	//element.append('<input type=hidden name=selected_patient class=selected_patient_input value=' + selected_pt + ' />');
								//element.submit();
	//$('.div_shower').dialog("close");
								//location.reload(true);
	...
	$('#pt_update_changes').click(function() {
		...
		if(segmentx !='' && religionx !=''){
			...
			$.ajax({
				...
				data:{ selected_upd: selected_pt , religion_upd : religionx , segment_upd: segmentx,action: actionx},
				success: function(response){
					//alert('record ikosawa amaccccccc: '+ response);
					resp = true;
			...
			//alert('record resp: '+resp);
			if(resp == true){
				//alert('record ikosawa ama: '+resp);
				allocate_patient_to_waiting_list(udpId);
			}
...
function show_seg_relig_values(){

}
function udt_patient_dg(){
	var seg = $("#first_name2").val();
	var relig = $("#middle_name2").val();
	
}
function fill_patient_dg(fname,mdname,lname,gender){
	//alert('Function call found patient record : firstname : '+ fname + " midname: "+ mdname + " lastname :"+ lname );
...
// allocate patient after search

function allocate_patient_to_waiting_list(udpId){
	var action = "allocate_pt_record";
	var selected_upd = udpId;
	//alert('Function call for allocating patient to waiting listxxxxxxx'+ udpId );
	...
	$.ajax({
		method: "GET",  
		 ...
		 data: {action:action, selected_patient:selected_upd}, 
	   	success: function(response){
					 //alert('patient allocated correctlycccccc' ); 					 			 
...
//search_patient_segment_religion();
//check if selected patient has segment and religion filled
//var udpId = '';


// segment and religion not filled so dispay fields to fill

function patient_seg_relig_empty(udpId){
	var action = "fill_pt_record";
	var selected_upd = udpId;

	$.ajax({
		method: "POST",  
		 url: "../../dental_includes/search_for_patient_2.php", 			   
		 dataType: "json",
		 data: {action:action, selected_upd:selected_upd}, 
	   	success: function(response){
			...
			if(typeof x2[1] !== 'undefined'){
				...
				$('.selected_pt').click(function() {
					...
					var selected_pt = $(this).prev().val();
						...
						alert('selected is BBBBBBBBBBBB  ' + selected_pt);

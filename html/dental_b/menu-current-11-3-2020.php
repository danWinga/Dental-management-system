			...
			if(typeof x2[1] !== 'undefined'){
				...
				$('.selected_pt').click(function() {
					...
					if(selected_pt.trim() !==''){
						...
						//var pt_data = search_patient_segment_religion(udpId);
						
						//var data = JSON.parse( pt_data);
						...
						$.ajax({
							...
							url: "dental_b/", 			   
							dataType: "json",
							data:{ selected_upd: selected_upd , action: action},
							success: function(response){
							...
							for (var i = 0; i < recordFound.length; i++) {
								...
								idp  = (recordFound[i].pid).trim();
								religion = recordFound[i].religion.trim();
								segment  = recordFound[i].segment.trim();
									
							}
							//alert('found patient record are: idp : '+ idp + " segment: "+ segment + " religion :"+ religion );
							if((religion.trim() !=''|| !religion.trim()) &&(!segment.trim() || segment.trim() !='')){
								element.append('<input type=hidden name=selected_patient class=selected_patient_input value=' + selected_pt + ' />');
								element.submit();
								$('.div_shower').dialog("close");
							}	
							else if(  religion.trim() =='' || segment.trim()==''){
								...
								$.ajax({
									...
									data:{ selected_upd: selected_pt , action: action},
									success: function(response){
							...
							$('#pt_update_changes').click(function() {
								...
								//alert('found patient rec are: segment : '+ segment + " religion: "+ religion + "selected_id: "+selected_pt );
...
//var udpId = '';
function search_patient_segment_religion(udpId){
	var udpId = 'jshfdjfdhfdjfdhfdjfdhfddfdfd';
	var action = "check_upd_action";
	var selected_upd = udpId;
	// ata: {action2:action, selected_upd:selected_upd}, 
	$.ajax({
		method: "GET",  
		 url: "treatment-plan/encrypt_id.php", 			   
		 dataType: "json",
		 data: {selected_pt: selected_upd}, 
	   success: function(response){
		alert('selected is yyyyy rereerere  ' + response);
		  var recordFound = response;
		  alert('selected is yyyyy rereererecccccccccccccc  ' + recordFound);
		}   
	  });
	return recordFound; 
}

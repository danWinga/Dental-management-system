...
if(!userIsLoggedIn()){echo "xxxxxxxxxxxxxxxxxxxxx";}
...
<div class=grid-container>
	...
	<input type="hidden" name="MAX_FILE_SIZE" value="2097152"/>
			...
			<div class='grid-75'>
				...
				<?php if($_SESSION['card_issued']=='YES'){ $yes_selected=" selected ";$no_selected="";}
					...
					<option value='YES' <?php echo $yes_selected; ?> >YES</option><option value='NO' <?php echo $no_selected; ?> >NO</option></select></div>
						...
						<!-- Patient Segmant -->
			<div class='grid-15'><label for="" class="label">Segment</label></div>
				<div class='grid-25'><select name=patient_segment><option>
				<?php
					$sql=$error=$s='';$placeholders=array();
			...
			<div class='grid-25'>
				...
				<?php if(!isset($_SESSION['photo_path']) or $_SESSION['photo_path']==''){$_SESSION['photo_path']="/profile/patient_photo.png";}?>
		...
		<fieldset><legend>Other Details</legend>
				...
				<!--city-->
				<div class='grid-15'><label for="" class="label">City/Town</label></div>
				...
				<div class='grid-25'><select name=referee><option></option>
				<?php
					$sql=$error=$s='';$placeholders=array();
				...
				<div class='grid-15'><label for="" class="label">Religion</label></div>
				<div class='grid-15'><select name=religion><option></option>
				<?php
					$sql=$error=$s='';$placeholders=array();
				...
				<div class='grid-15'><label for="" class="label">Source</label></div>
				<div class='grid-15'><select name=patient_source><option></option>
				<?php
					$sql=$error=$s='';$placeholders=array();
					$sql = "select id,name from patient_source order by name";
					$error = "Unable to list patient source";
					$s = 	select_sql($sql, $placeholders, $error, $pdo);					
					foreach($s as $row){
						$name=html($row['name']);
						$val=$encrypt->encrypt(html($row['id']));
						//echo "<option value='$val'>$name</option>";
						if($_SESSION['patient_source']==$row['id']){echo "<option value='$val' selected>$name</option>";}
						else{echo "<option value='$val'>$name</option>";}						
					}
				...
				<div class='grid-25'><input type=text name=other_name value='<?php echo $_SESSION['behalf_name']; ?>' /></div>						
				<!--other relationship-->
				<div class='grid-15'><label for="" class="label">Relationship</label></div>

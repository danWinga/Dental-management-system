<?php
	...
	//include('../dental_includes/encryption.php');
//	include('../dental_includes/magicquotes.inc.php');
...
//	include_once   '../dental_includes/DatabaseSession.class.php';
	...
	//include('../dental_includes/fpdf/fpdf.php');
	include_once('/dist/fetch.php');
...
if(isset($_SESSION['token_sr']) and isset($_POST['token_sr']) and $_POST['token_sr']==$_SESSION['token_sr'] and isset($_POST['surgery_name'])
	and $_POST['surgery_name']!=''){
			...
			$s = 	insert_sql($sql, $placeholders, $error, $pdo);
			if($s){
				//ensure a surgery was selected
				if($_POST['branch_name'] == ''){$_SESSION['select_surgery']='no';}
				else{
					$_SESSION['branch'] = $encrypt->decrypt($_POST['branch_name']);
					if($_SESSION['branch'] == 0){$_SESSION['branch_name']='E-House';}
					elseif($_SESSION['branch'] == 1){$_SESSION['branch_name']='Lenana';}
					$_SESSION['select_surgery']='';}
			}
}
...
<!-- added by dan -->
<link rel="stylesheet" href="dental_procedure_upgrades/css/bootstrap3.3.0_r.css">
...
<!--enddd -->
<!--
 <script type="text/javascript" charset="utf8" src="dental_b/
dist/cdnjs.cloudflare.com/ajax/libs/bootstrap-treeview/1.2.0/bootstrap-treeview.min.js"></script>
 <link rel="stylesheet" type="text/css" href="
 dental_b/dist/treeview_css/cdnjs.cloudflare.com/1.2.0/bootstrap-treeview.min.css"/>
-->



<!------ links for tab bootstrap ---------->
...
echo "</div>";//this is for main_content	
	...
	elseif(isset($_SESSION['id']) and (!isset($_SESSION['branch']) or  $_SESSION['branch']=='')){ 
		...
		<div class='grid-100 grid-parent'>
			...
			<div class="prefix-33 grid-33 suffix-33" id="login">
				...
				<label for="username" class="login_label">Select your branch</label></div>
				<div class="grid-80" ><select name=branch_name>
					<?php 
						$e_house=$encrypt->encrypt('0');//e-house
						$lenana=$encrypt->encrypt('1');//lenanan
						echo "<option value=''></option>";
						echo "<option value='$e_house'>Electricity House</option>";
						echo "<option value='$lenana='>Lenana</option>";
				...
				<?php 
					//for doctors show them surgery tp select
				if($_SESSION['is_user_doctor']==1 and  $_SESSION['select_surgery']=='no'){ ?>
				<div class="grid-100"><label for="username" class="login_label">Select the surgery your working from</label></div>
				...
				<?php }
					//for none doctors let their surgery be noen
				else{
					$val=$encrypt->encrypt('0');	
					echo "<input type=hidden name=surgery_name value='$val' />";
				}
				?>
				<div class=clear></div>
...
<div class="mainsss">
		<div class="clear"></div>
		 <div  class="main_menu"> 
			...
			?>
			<li><a href=?id=log-out>Log Out <?php htmlout("$_SESSION[user_name] / $_SESSION[branch_name]"); ?></a></li>
			</ul><!-- end nav1 -->
			</div>
		 </div>
		 </div>
...
if($_GET['id']=='procedure-done-per-doctor'){if(userHasRole($pdo,123)){include_once     '../dental_includes/procedure_done_per_doctor_report.php';}
														  elseif(!userHasRole($pdo,123)){?><div class="grid-100 no_js_text">ERROR: ACCESS DENIED!</div><?php }
														}														  
												//echo "</div>";//container														
